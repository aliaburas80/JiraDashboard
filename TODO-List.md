...
**Last updated:** 2026-06-08 (TRACE-01 gap clusters #1, #2, #3, and #4 fully closed plus UX-14 — F1-07/08 throughput data-contract anchored to UC-043/SCN-012/UJ-010 and TC-T-11 automated, 509/54 passing)  
...
| `product/TEST_CASES.md` | Yes | Yes | Replaced the stale `### F4 — Excel Export Tests (manual)` table (6 rows, IDs `TC-X-01–06`, all "Not Run" — none of which matched the actual automated suite) with a corrected `### F4 — Excel Export Tests (automated — excelExport.test.ts, excelExportSheets.test.ts)` table listing all 18 real `TC-X-01–13b` cases as ✅ Pass, plus a `**Related:**` line pointing at `UC-049`, `UC-089`, `SCN-016`, `SCN-045`, `UJ-013`, `UJ-029`, `FR-236`, `FR-242`, `FR-243`. | Done |
| `product/DEVELOPER_GUIDE.md` | Yes | No | Searched for new ID-range / "through" index phrases — no references to UC/SCN/UJ/TC/FR ID ranges found; nothing to update. | Done |
| `product/RELEASE_NOTES.md` | Yes | Yes | Inserted new top section `## v4.2.2 — TRACE-01 Cluster #3 Closure: Smart Excel Export Sheets & Trigger (2026-06-08, P0 — documentation + test coverage)` describing: `FR-242`/`FR-243` additions, `UC-089`/`SCN-045`/`UJ-029`, the 10 new `TC-X-09a–13b` automated tests, the corrected stale `TEST_CASES.md` table, and the suite growth 498/53 → 508/54. | Done |
| `product/README.md` | Yes | No | No UC/SCN/UJ/TC/FR ID-range references found. | Done |
| `product/ALGORITHM_SPEC.md` | Yes | No | Algorithm/formula spec already documents the percentile formula at the implementation level; no UC/SCN/UJ/TC/FR ID-range references found needing update (0 hits for UC-0/SCN-0/UJ-0/TC-X/FR-242/FR-243). | Done |
| `product/TECHNICAL_METHOD.md` | Yes | No | Patent/technical-method document; no UC/SCN/UJ/TC/FR ID references found (0 hits). | Done |
...
| ID | Task | Priority | Status | Details / Acceptance Criteria |
|---|---|---:|---|---|
| TRACE-01 | Build full traceability matrix for every shipped v4.2.x feature | P0 | 🔧 In progress — clusters #1 (F3-14/15/16), #2 (F2-05/06/07/09/11/12/13), #3 (F4-05/06/08), #4 (F1-07/08), and UX-14 fully closed 2026-06-08 incl. all 34 test cases (14 + TC-AC-01–03 + TC-FF-01–06 + TC-X-09a–13b + TC-T-11) automated | Cross-reference Feature ↔ SRS FR ID ↔ Use Case ID ↔ Scenario ID ↔ User Journey ID ↔ Test Case ID ↔ Release Note ↔ TODO Status. Filled matrix and prioritized gap punch-list now in Section 12. Cannot be marked ✅ Done until the remaining UX narrative gaps (UX-02/03/05/11/13) and the FR↔UC bundling ambiguity (item 6) are closed (see Gaps Summary). |
...
| F4-05 Risks & Blockers / Orphan & Data Quality / Release Readiness sheets | FR-236, FR-242 | UC-089 | SCN-045 | UJ-029 | TC-X-09a, TC-X-09b, TC-X-10a, TC-X-10b, TC-X-10c, TC-X-12 | v3.0 — sheet bullet; v4.2.2 — TRACE-01 cluster #3 closure (2026-06-08) | ✅ Done — traceability closed 2026-06-08, all 6 sheet-content TCs automated in `excelExportSheets.test.ts` |
| F4-06 Cycle & Lead Time percentile analysis | FR-236, FR-242 | UC-089 | SCN-016, SCN-045 | UJ-029 | TC-X-11a, TC-X-11b | v3.0 — "Cycle & Lead Time, Throughput Trends..."; v4.2.2 — TRACE-01 cluster #3 closure (2026-06-08) | ✅ Done — traceability closed 2026-06-08, percentile + slowest-items logic re-verified and automated |
| F4-07 Metric Dictionary sheet | FR-240 | UC-049 | GAP — not found | UJ-013 | TC-X-05 | v3.0 — "Metric Dictionary, Raw Data Reference" | ✅ Done |
| F4-08 Export button triggers smart workbook | FR-236, FR-243 | UC-049, UC-089 | SCN-016, SCN-045 | UJ-013, UJ-029 | TC-X-13, TC-X-13b | v3.0 — "Export button in dashboard sticky bar and summary page"; v4.2.2 — TRACE-01 cluster #3 closure (2026-06-08) | ✅ Done — traceability closed 2026-06-08, trigger flow (`exportToExcel`, default + custom filename) automated in `excelExportSheets.test.ts` |
...
5. ~~**UX-14 (flat admin-settings redesign)**~~ — **FULLY CLOSED 2026-06-07.** Anchored with `UC-087`, `SCN-043`, `UJ-027`, `TC-AC-01–TC-AC-03`, then **automated all 3 previously-❌-Not-Run test cases the same day**: `TC-AC-01` (sidebar tab metadata via `activeTabMeta`/`ADMIN_TABS`), `TC-AC-02` (tab-switch panel/stat-card swap via `activeTabMeta`/`buildSettingsStats`), `TC-AC-03` (Users-tab summary cards, `roleOptionsFor`, `matchesUserFilter` table search/filter) — new `adminSettingsConsole.test.ts`, with `Tab`/`ADMIN_TABS`/`activeTabMeta`/`retentionLabel`/`buildSettingsStats`/`ManagedUser`/`roleOptionsFor`/`matchesUserFilter` extracted to `src/lib/adminConsole.ts` for direct unit testing (mirrors the `src/lib/members.ts` pattern). Test count rose from 481/51 to 492/52 suites. **Remaining UX items (UX-02, UX-03, UX-05, UX-11, UX-13)**: "Default open sections", "status chips", "HTML export redesign", and "advanced theme/chart customization" still lack UC/SCN/UJ/TC anchoring. **Action**: backfill narrative coverage for these remaining UX items.
...
**Net assessment (updated 2026-06-08):** Gap clusters #1 (F3-14/15/16), UX-14, #2 (F2-05/06/07/09/11/12/13), #3 (F4-05/06/08 — Excel export sheets and trigger), and now #4 (F1-07/08 — throughput types & `DashboardMetrics.throughput` data contract) are **all fully closed**: documentation anchored across all five clusters (4+2+2+0 new FRs — cluster #4 reused `FR-215` and extended `UC-043`'s range — plus 3 new UCs, 3 new SCNs, 3 new UJs and 1 new `**Related:**` line on `SCN-012`), F2-11/12/13 re-verified at the code level and promoted from 🔍 to ✅, and 34 new test cases (14 + `TC-AC-01–03` + `TC-FF-01–06` + `TC-X-09a/b/10a/b/c/11a/b/12/13/13b` + `TC-T-11`) automated across the five passes. Test suite has grown from 469/48 → 481/51 → 492/52 → 498/53 → 508/54 → **509/54** across the five closures; lint and build remain clean throughout. The only remaining matrix gap cluster is the residue of #5 (UX-02/03/05/11/13 narrative coverage), plus the cross-cutting FR↔UC bundling ambiguity (item 6). TRACE-01 cannot be marked ✅ Done until those are closed too. TRACE-02 (full app coverage validation) should follow the same evidence-based approach once this matrix's gaps are filled, since many `COVER-*` rows depend on the same UC/SCN/TC inventory being complete.
...
| F4-TRACE | Add/verify traceability for all Feature 4 items | P0 | ✅ Done — traceability closed 2026-06-08: F4-05/06/08 anchored with `FR-242/243`/`UC-089`/`SCN-045`/`UJ-029`, and `TC-X-09a–TC-X-13b` (10 new sheet-content + trigger-flow cases) automated in `excelExportSheets.test.ts` (see Section 12 Gaps Summary item 3 — fully closed) |
...
| UX-TRACE | Add/verify traceability for all UX items | P0 | 🔧 In progress — UX-14 (flat admin redesign) fully closed 2026-06-07 (`UC-087/SCN-043/UJ-027/TC-AC-01–03`, all automated); UX-02/03/05/11/13 still lack anchoring — see Section 12 Gaps Summary item 5 |
...
|---|---|---:|---|---|
| NEXT-01 | Complete `TRACE-01` full traceability matrix | P0 | 🔧 In progress — clusters #1 (F3-14/15/16), #2 (F2-05/06/07/09/11/12/13), #3 (F4-05/06/08), #4 (F1-07/08), and UX-14 fully closed 2026-06-08 incl. 34 automated tests (14 + TC-AC-01–03 + TC-FF-01–06 + TC-X-09a–13b + TC-T-11); only UX-02/03/05/11/13 narrative anchoring and the FR↔UC bundling ambiguity remain | This is the main blocker before new P1/P2 development. |
