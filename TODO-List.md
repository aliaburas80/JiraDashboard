...
| JIRA-09 | Tests | P1 | 🔧 Partially Done (2026-06-21) | `jiraConnections.test.ts` — 17 tests (`TC-JIRA-01–13`, `25–28`) covering all three Jira admin routes (create/list, test-connection, field discovery): auth/role guards, validation, create + audit, test success (Cloud Basic + Server/DC Bearer), gateway failure handling, token-not-configured guard. New `jiraApiAdapter.test.ts` — 11 tests (`TC-JIRA-14–24`) covering standard-field mapping, custom-field resolution via `fieldMapping`, Sprint/Epic Link shape normalization, standard-field precedence, array ordering, and a direct `validateIssueData()` pass-through check. Plus `TC-GW-22`/`22b`/`23`/`23b` for JIRA-05b/05c. **Still needed:** sync-route tests once JIRA-07 exists. Suite: 604/65 passing. |
