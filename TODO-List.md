...
| DEP-UPGRADE-NEXT16 | Major upgrade: Next.js 14.2.35 → 16.x | P2 | ❌ Not started | Deferred out of `QA-GATE-03`'s scope — a major-version upgrade, not a security patch. Required to fully close the two remaining high-severity Next.js DoS advisories (GHSA-9g9p-9gw9-jx7f image-optimizer DoS, GHSA-h25m-26qc-wcjf RSC HTTP-deserialization DoS) that persist even at 14.2.35. Also pulls React 18→19, `eslint-config-next` 14→16 (closes the `glob`/`minimatch`/`@typescript-eslint` CVE chain currently blocked behind this same bump), and raises the Node.js minimum to 20.9. Follow CLAUDE.md §4.8's full major-update process: dedicated upgrade branch, official Next.js migration guide + codemods, architecture review, full regression, dependency compatibility testing, ADR. Do not start opportunistically inside an unrelated task. |
| DEP-DEFERRED-01 | Tracked, accepted dependency risks not fixed in the 2026-07-22 pass | P2 | ⚠️ Accepted risk — reviewed 2026-07-22 | `nodemailer` 8.x has a high-severity SSRF/arbitrary-file-read CVE (GHSA-p6gq-j5cr-w38f); fix requires a major bump to 9.0.3 which may change call signatures in email-sending code — needs dedicated review, not a today-fix. `xlsx` (SheetJS) has a high-severity Prototype Pollution + ReDoS pair (GHSA-4r6h-8v6p-xvw6, GHSA-5pgg-2g8v-p4x9) with **no upstream fix available at all** — real, currently-live exposure on the Excel-import path; options are accepting the risk with input-size/content hardening at the call site, or replacing the library, neither decided yet. `@google-cloud/storage`'s `uuid`/`gaxios`/`teeny-request` chain (GHSA-w5hq-g745-h8pq, moderate) is fixable only via a major downgrade to `@google-cloud/storage@5.18.3` — deferred alongside the others rather than force-installed unreviewed. Re-run `npm audit` at the next dependency-health check (`QA-GATE-03`) rather than assuming this list is still current. |
