...
| DOC-GATE-09 | Review patent docs when novelty/claim/prior-art positioning changes | P0/P2 | 🚫 Blocked on inventor review (2026-08-08) | Fact-checked each `PATENT_DISCLOSURE.md` §4 claim against the current implementation (evidence in that file's new "Current Code Alignment — 2026-08-08" section) — deliberately did not edit any claim/legal language, since claim-scope decisions belong to the inventor, not this pass. Most claims verified accurate (hierarchy reconstruction's exact confidence scores, the 17-sheet Excel export, zero-credential upload path, React Flow/Dagre explorer). **Two real, material discrepancies found**, both needing the inventor's own decision before filing: (1) §4.4's four named orphan sub-categories (`MISSING_EPIC`/`MISSING_PARENT`/`DANGLING_LINK`/`FULLY_ORPHANED`) don't exist in code — only a single `isOrphan` boolean with one general impact statement. (2) §4.5's health-score formula (weights and even which signals are used) doesn't match `calculateHealthScore()` as actually shipped — different weights throughout, no `blocked_ratio` term in code, and the claimed "velocity_trend" signal is actually the latest sprint's raw completion rate. `PRIOR_ART_COMPARISON.md`/`CLAIM_CANDIDATE_MATRIX.md` not yet cross-checked against code — out of scope for this pass. |
