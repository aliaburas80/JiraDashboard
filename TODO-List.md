...
| ARCH-05 | Jira API read-only architecture | P2/P0 | ✅ Design Done (2026-06-20) — awaiting approval to implement | Design doc written: `product/JIRA_INTEGRATION_DESIGN.md`. Covers auth model (API token/PAT via the existing FR-313 Gateway `jira` blueprint), API scope (JQL search, guided filters not raw JQL), field mapping (extends the existing column-mapping pattern one layer earlier, converging to the same canonical `JiraIssue[]` shape), refresh tiers (manual → scheduled → webhook, only manual ships in Phase 1), new `JiraConnection` Prisma model + `ImportLog.sourceType`/`jiraConnectionId` columns, failure-mode table, and fallback behavior mirroring `loadMetricsWithSource()`. **No code implemented — design only, per the "do not implement without explicit approval" rule for this section.** |
