...
| QA-GATE-09 | Fix the `e2e` CI workflow, which had never actually passed since it was introduced (`QA-GATE-05`/`06`, 2026-07-22) | P0 | 🟡 Both original bugs fixed and confirmed; a third, previously-unreachable issue found — 2026-07-25 | **Both root-caused bugs below are fixed and confirmed working in real CI** (`gh run view` on PR #21's actual Actions run, not just local): the app now builds and boots in the `e2e` job for the first time ever — real progress, since it always died on Bug 1 before. Root-caused via `gh run view --log-failed` on the 3 most recent PRs (#15, #17, #19) — same failure every time, unrelated to any of those PRs' own changes, confirming the pipeline itself was the bug, not the code it was gating. **Bug 1:** `.github/workflows/e2e.yml` set `NODE_ENV: production` at job level, active during `npm ci` too — `npm ci` respects `NODE_ENV=production` and skips `devDependencies`, where `tailwindcss` lives, so `next build` failed immediately with `Cannot find module 'tailwindcss'`. Fixed by simply deleting the line — Next's own CLI already defaults `NODE_ENV` to `production` for `next build`/`next start` when unset (confirmed by reading `node_modules/next/dist/bin/next`), so no functional loss; matches how `quality.yml` (which never set this line) already builds cleanly. **Bug 2, latent, would have surfaced immediately after fixing Bug 1:** the workflow's `STORAGE_DRIVER: temporary` (fine for an ephemeral single-use CI Postgres container) is unconditionally rejected by `scripts/start-production.mjs`'s `validateEnvironment()` — the real `npm run start` entrypoint, also what Playwright's `webServer` boots — and by the identical guard in `src/lib/env/server.ts`'s `getServerEnv()` (called broadly at runtime), both by design: Render's disk is genuinely ephemeral in real production, so this guard exists to prevent silently losing uploaded files/backups (`product/ERRORS.md` ERR-004). Fixed with a narrow, explicit, **doubly-gated** CI-only bypass — `ALLOW_TEMPORARY_STORAGE_IN_CI=true`, only honored when `process.env.CI === 'true'` is *also* true (set automatically by GitHub Actions, never present on Render) — added to both files via a shared `isEphemeralCiRun()` check, with the flag set only in `.github/workflows/e2e.yml`. New `src/__tests__/serverEnv.test.ts` (5 tests) proves the guard still throws with either flag alone and only passes with both, and that the bypass doesn't relax the unrelated `DATABASE_URL`-must-not-be-`file:` check. `product/ERRORS.md` ERR-004 documents the exception inline. Full verification: `typecheck` clean, `lint` 8/8 (unchanged pre-existing warnings), `lint:css` clean, `test` 114/114 suites / 1087/1087 tests (+5 new), `build` clean. **Bugs 1-2 fully resolved and confirmed in real CI** — the app now builds and boots in the `e2e` job every time. Three more layers surfaced once real traffic finally reached them for the first time ever, chased across 5 real CI runs (each read via `gh run view`/downloaded Playwright reports, not guessed):

**Bug 3 (fixed, confirmed):** the suite's own job `timeout-minutes: 20` killed the run mid-suite with zero diagnostics (no Playwright report artifact even got uploaded) — not a hang, confirmed via the job's own step timestamps showing `cancelled`, not `failure`. Root cause: `mobile-dense-tables.spec.ts`/`mobile-forms.spec.ts`'s own file-header comments said "run against the Mobile project only," but nothing enforced that, so they silently also ran on Desktop Chrome/Firefox/Safari/Tablet — 20 full login+upload cycles where 8 were intended. Fixed via `testIgnore` scoping in `playwright.config.ts` (`MOBILE_ONLY_SPECS`) plus `timeout-minutes: 20→30` as headroom. Confirmed: the next run completed cleanly (`failure`, not `cancelled`) and uploaded a real report.

**Bug 4 (fixed, confirmed):** with a real report in hand, found every test *after the first* failed deterministically at the exact same login step, while the first test failed later (at upload) — the signature of shared mutable state, not flakiness. All 8 tests share one seeded admin row in the same CI Postgres database; the first test to run completes the forced password-change flow and permanently rotates the account's real password, but `helpers/auth.ts` kept trying the original constant. Fixed by tracking the account's actual current password in a module-level variable (safe because `workers:1`/`fullyParallel:false` means the whole suite runs sequentially in one persistent process) rather than a guess-and-fallback dance, which itself briefly introduced a second real bug (doubled login POSTs from the guessing exhausted the login route's 5-attempts/60s rate limiter, since every request in this CI setup shares one 'unknown'-IP bucket with no reverse proxy — a 429 looks identical to a wrong password from the test's point of view, which is why that first attempt looked like it hadn't worked).

**Bug 5, newly discovered, NOT yet fixed — the one still blocking green:** the very first upload request in every run (test 1, always) never completes within its 40s budget, and — critically — appears to permanently wedge the server: every test that runs after it, for the rest of that same CI job, fails immediately at the login step (never mind upload), even once Bug 4's password-tracking fix was in place and confirmed correct for the one test that got to try it. This pattern is 100% deterministic across all 3 runs since Bug 4's fix (test 1 dies at upload; every test after it dies at login) — not the profile of flakiness or cold-start variance, but of a single request holding some shared resource forever (leading hypothesis: a Prisma connection or open `$transaction` in the upload route, `app/api/upload/route.ts`, never released — possibly `getWorkspaceForUser()`'s awaited call at the metrics-save step, or the `prisma.$transaction` block creating the `ImportLog` row — genuinely hanging rather than erroring, since `.catch(() => {})` handlers on those calls never fire if the underlying promise simply never settles). Not chased further without checking in first: this is no longer a CI-config bug, it's a potential real backend reliability issue (if a hung upload can wedge Render's production process the same way, that's a genuine incident risk, not just a test problem), each further diagnostic round costs a full ~15-20 min CI cycle, and 5 rounds have already been spent getting this far. Next step if resumed: temporary server-side timing instrumentation around each awaited step in the upload route's success path to pinpoint exactly which call never returns, rather than guessing further. |
...
| P0A-02 | Upload pipeline reliability | P0 | 🟡 Partially done — re-scoped 2026-07-25 | Tracker was stale — code-level audit found file-size limits (`MAX_UPLOAD_MB`, validated), magic-byte content-vs-extension validation (`src/lib/fileSignature.ts`), clean try/catch error handling with entitlement-safe rollback, and 33+ existing tests across `jiraParser.test.ts`/`fileSignature.test.ts`/`uploadUserId.test.ts`/`columnMapping.test.ts` all real and working. Genuine remaining gap: single-file `/api/upload` doesn't dedup by Issue Key (multi-file merge's `mergeIssues.ts` does) — a repeated row in one CSV double-counts. Real remaining work: reuse `mergeIssues.ts` dedup in `/api/upload` + add tests; unit-test `src/services/jira/validation.ts` (currently zero direct tests); add a 413-limit test, a corrupt-past-signature-check test, and an empty-CSV test — **~4-6 hrs, tracked as Phase 2 of the P0-A gate work, not done in this pass.** |
...
| P0A-04 | Data isolation and workspace security | P0 | 🟡 Partially done — re-scoped 2026-07-25 | Tracker was stale — real per-user/workspace scoping exists (`userId`+`workspaceId` on every relevant Prisma model and query), IDOR-hardened (404 not 403 on cross-account access attempts), and 26 existing negative-access tests already pass (`src/__tests__/crossAccountDataIsolation.test.ts` TC-ISO-01–08, `workspaceIsolation.test.ts` TC-DATAISO-01–18). Note: today's isolation boundary is per-user (one `Workspace` per `User`, MVP design) — true multi-org tenancy is the separate, unmerged `feature/org-phase1-tenant-isolation` branch tracked under §20a, not this item. Real remaining work: audit ~7 admin routes to confirm intentional global-admin-only scope (not missed `userId`/`workspaceId` filters), add 2 more negative tests on export endpoints — **~4-8 hrs, tracked as Phase 2, not done in this pass.** |
...
| P0A-06 | Database production readiness | P0 | 🟡 Partially done — re-scoped 2026-07-25 | Migrations (19, sequential/consistent) and indexes (`@@index` on `userId`/`workspaceId`/`email`/etc.) are genuinely solid — that half of "partially done" was accurate. **Real, more serious gap than the tracker implied:** the in-app "Backup & Restore" admin feature (`src/services/settings/backup.service.ts`) and `product/DEPLOYMENT_GUIDE.md` §11's cron example both back up a **SQLite file (`delivery_clarity.db`) that no longer exists** — the schema has been Postgres/Neon since the DEP migration, so backup is silently a no-op against real production data today, not just "unverified." No `pg_dump`/`pg_restore` script exists anywhere. Real remaining work: build a real Neon-native Postgres backup + restore runbook — **~2-3 days, the single largest P0-A gap found, tracked as Phase 3 (own risk review per CLAUDE.md §53 High Risk — touches the production data-recovery path), not done in this pass.** |
| P0A-07 | Audit and operational logging | P0 | 🟡 Partially done — re-scoped 2026-07-25 | Tracker's claim is partly false: structured JSON logging + secret redaction (`scripts/start-production.mjs`) is real, but **"correlation IDs" do not exist anywhere in the codebase** (zero hits for `correlationId`/`requestId`/`x-request-id`) — corrected here since asserting something exists when it doesn't is worse than marking it not-started. `safeAuditEvent` (`src/lib/system-error-logger.ts`) covers most sensitive admin actions (user create/role-change/delete, jira-connections, storage, cleanup, add-request accept/reject), but 2 destructive routes have none: `admin/users/[id]/reset` and `admin/security`. Real remaining work: add per-request correlation IDs threaded through logs (~1 day); add audit events to the 2 missing routes (~0.5 day) — **~1.5 days, tracked as Phase 3 (touches the shared logging utility every route calls through), not done in this pass.** |
...
| P0A-09 | Performance baseline | P0 | 🟡 Partially done — re-scoped 2026-07-25 | Tracker undersold what exists: `app/api/upload/route.ts` already persists real per-upload timing (`ImportLog.processingTimeMs`), and `product/RELEASE_NOTES.md` documents a prior informal ~5k-issue perf pass (parseDate memoization, `flowItemByKey` Map optimization). Genuine gap: timing only wraps metric calculation, not the parse/dashboard-render/export legs; no documented thresholds; `public/samples/sample-jira-export.csv` is only ~35 issues, nowhere near the 3k-7k target. Real remaining work: instrument the other legs (~2-4 hrs); script a synthetic 3k-7k row dataset generator (~1 hr); run + document thresholds (~2-3 hrs) — **~1 day, tracked as Phase 3, not done in this pass.** |
