...
| STYLE-07 | Switch `npm run lint` to the CLAUDE.md §4.6-mandated command | P0 | 🚫 Blocked | `package.json`'s `lint` script currently runs `next lint`, which §4.6 explicitly prohibits in favor of `eslint . --max-warnings=0`. Blocked on `STYLE-09` (the `app/**` Tier 4 drift): flipping it today would fail on the ~195 warnings still in those pages. `src/components/**` (`STYLE-02`–`06`) is now fully closed except one deliberately-accepted `SvgIcon.tsx` warning. Flip once `STYLE-09` closes, or agree on an interim ratcheting `--max-warnings` ceiling. |
| STYLE-09 | Re-remediate `app/**` Tier 4 drift | P1 | ❌ Not started | Found while closing out `STYLE-06`: `app/**` standalone pages that Tier 4 (`STYLE-05`) closed to ~30 warnings on 2026-07-19 have drifted back up to ~195 warnings as of 2026-07-22 — `app/forecast` (56), `app/sprint-kanban` (22), `app/charts` (13), `app/portfolio` (13), `app/customer` (12), `app/delivery-mix` (11), `app/roadmap` (9), and smaller amounts across a dozen more files. New feature work landed on these pages since Tier 4 closed and reintroduced inline styles. Explicitly deferred — owner chose to finish `STYLE-06` first and treat this as a separate follow-up rather than scope-creeping into `STYLE-06`. |
