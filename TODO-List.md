...
| JIRA-04 | Admin UI: connect / test / field-mapping | P1 | ❌ Not started | Next slice — admin form to create a `JiraConnection` (name, deployment type, base URL, email for Cloud), "Test connection" action calling `GET /rest/api/{2\|3}/myself` through the Gateway, and a field-mapping step reusing the column-mapping page's fuzzy-match UI sourced from `GET /rest/api/3/field` instead of CSV headers. |
| JIRA-05 | API routes: create/list/test connection | P1 | ❌ Not started | `POST /api/admin/jira-connections`, `GET /api/admin/jira-connections`, `POST /api/admin/jira-connections/[id]/test` — admin-only, credentials never returned in responses (`hasCredentials` flag pattern, matching `app/api/admin/storage/route.ts`). |
...
| JIRA-09 | Tests | P1 | ❌ Not started | Unit tests for `apiAdapter.ts` field mapping, route guard tests (admin-only, credential redaction) mirroring `userAddRequests.test.ts` patterns, gateway integration test for the `jira` provider's test-connection call. |
| JIRA-10 | Update all related product docs | P1 | ❌ Not started | SRS FR additions, USE_CASES (admin connects Jira / runs manual sync), DEVELOPER_GUIDE, RELEASE_NOTES, TEST_CASES — once the user-facing slice (JIRA-04+) exists. Schema-only commits (JIRA-01–03) intentionally skip UC/SCN/UJ per the "no UC for vaporware" principle, same as the original Gateway foundation. |
