...
| USERREQ-31 | Fix pre-existing test failures in `adminUsers.test.ts` / `roles.test.ts` | P1 | ❌ Not started | Discovered 2026-06-20 while fixing USERREQ-25/27 (unrelated file, not introduced by this pass): `DELETE /api/admin/users` calls `prisma.userAddRequest.updateMany(...)` (cancels pending requests for the deleted user's email) but the test mocks for these two files don't define `userAddRequest.updateMany`, causing a `TypeError`. Same class of bug as the mock drift fixed in `userAddRequests.test.ts` TC-REQ-01/04/10/13 — add the missing mock method. |
