...
| JIRA-04 | Admin UI: connect / test / field-mapping | P1 | ❌ Not started | Next slice — admin form to create a `JiraConnection` (name, deployment type, base URL, email for Cloud), wired to the JIRA-05 routes below, plus a field-mapping step reusing the column-mapping page's fuzzy-match UI sourced from `GET /rest/api/3/field` instead of CSV headers. **No UI exists yet — JIRA-05's routes are only reachable via API client/tests today.** |
...
| JIRA-05b | Gateway enhancement: per-connection `baseUrlOverride` | P1 | ✅ Done (2026-06-20) | Discovered while building JIRA-05: `callExternal()`/`getProviderConfig()` only supported a single global base URL per provider type (one env var), but `JiraConnection` allows multiple admin-configured connections. Added optional `baseUrlOverride`/`extraAllowedHosts` to `GatewayRequestOptions` and a 2nd `overrides` param to `getProviderConfig()` — additive, backward-compatible (existing single-arg callers and all `TC-GW-*` tests unaffected). Credential *values* still always come from env, never overridden — matches design doc §9's planned extension. New tests `TC-GW-22`/`TC-GW-22b`. |
| JIRA-06 | `src/services/jira/apiAdapter.ts` — normalize API response to canonical `JiraIssue[]` | P1 | ❌ Not started | Converts raw Jira REST issue objects to the same shape `parseJiraFile()` produces, using the per-connection `fieldMapping`, so `validateIssueData()`/`calculateDashboardMetrics()` need zero changes. |
| JIRA-07 | Manual "Sync now" route | P1 | ❌ Not started | `POST /api/admin/jira-connections/[id]/sync` — runs JQL via `callExternal()`, normalizes via JIRA-06, writes `ImportLog` (`sourceType: "api"`) + `DashboardSnapshot`, updates `lastSyncAt`/`lastSyncStatus`/`lastSyncError`. All-or-nothing per design §7 (no partial-page commits). |
| JIRA-08 | Fallback contract: `'jira-api'` `MetricsDataSource` | P1 | ❌ Not started | Extend `src/lib/storage.ts`'s `MetricsDataSource` union + `loadMetricsWithSource()` per design §8; source-status badge shows "Jira (name) — last synced Xm ago" on sync failure, never wipes last-good snapshot. |
| JIRA-09 | Tests | P1 | 🔧 Partially Done (2026-06-20) | `src/__tests__/jiraConnections.test.ts` — 13 tests (`TC-JIRA-01–13`) covering both JIRA-05 routes: auth/role guards, validation, create + audit, test-connection success (Cloud Basic auth + Server/DC Bearer auth, both verified via mocked `callExternal`), gateway failure handling, token-not-configured guard. Plus `TC-GW-22`/`22b` for JIRA-05b. **Still needed:** `apiAdapter.ts` field-mapping tests once JIRA-06 exists. |
| JIRA-10 | Update all related product docs | P1 | ❌ Not started | SRS FR additions, USE_CASES (admin connects Jira / runs manual sync), DEVELOPER_GUIDE, RELEASE_NOTES, TEST_CASES — once the user-facing slice (JIRA-04+) exists. Schema-only/API-only commits (JIRA-01–03, 05, 05b) intentionally skip UC/SCN/UJ per the "no UC for vaporware" principle, same as the original Gateway foundation — there is still no admin-reachable UI. RELEASE_NOTES + TEST_CASES updated for this slice; SRS/USE_CASES deferred to JIRA-04. |
| DRIFT-01 | Pre-existing migration drift: `SystemErrorLog` has no tracked migration | P2 | ❌ Not started (flagged, not fixed) | Discovered 2026-06-20 while migrating for JIRA-03: the dev DB's `SystemErrorLog` table exists (created via `db push` at some point) but no migration file ever recorded its creation, so `prisma migrate diff` tried to re-`CREATE TABLE` it. Scoped my migration to exclude it. **Risk:** a fresh environment running `prisma migrate deploy` from a clean DB would never get this table. Needs its own small migration (`CREATE TABLE IF NOT EXISTS` or a proper baseline) — out of scope for ARCH-05, tracked here so it isn't lost. |
