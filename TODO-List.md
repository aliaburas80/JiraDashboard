...
| JIRA-13 | Discovered, not fixed: relation graph never renders connecting edges | P2 | ❌ Not started (flagged, not fixed) | Found while verifying `JIRA-12` live: `WorkItemGraph.tsx`'s React Flow instance never renders any edge path — `.react-flow__edges` always contains only the `<defs>` arrowhead marker and an empty `<g>`, in both `next dev` and a production build, for graphs that were already known-correct before today (e.g. `AJ-28`'s 19 real children). `graph.edges` itself is correct (verified directly via `buildRelationGraph()` in a Jest test) and is passed through `applyDagreLayout()` to `setRfEdges()` unchanged. Likely a React Flow / dagre integration issue (node-handle positions, edge state timing, or a version mismatch) rather than a data problem. Out of scope for `JIRA-11`/`JIRA-12` (the user's actual report — missing ancestor *nodes* — is fully fixed); needs its own investigation. |
