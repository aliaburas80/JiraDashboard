...
| DOC-GATE-09 | Review patent docs when novelty/claim/prior-art positioning changes | P0/P2 | 🚫 Blocked on inventor review (2026-08-08) | Fact-checked all three required documents against the current implementation — `PATENT_DISCLOSURE.md`, `CLAIM_CANDIDATE_MATRIX.md`, and `PRIOR_ART_COMPARISON.md` — each now has a dated "Current Code Alignment — 2026-08-08" section with the evidence. Deliberately did not edit any claim/legal/positioning language itself — claim-scope decisions belong to the inventor, not this pass. Most claims verified accurate (hierarchy reconstruction's exact confidence scores, the 17-sheet Excel export, zero-credential upload path, React Flow/Dagre explorer). **Three discrepancies found, all needing the inventor's own decision before filing** — the two material ones appear in the draft claim text itself (`CLAIM_CANDIDATE_MATRIX.md`'s C-03/C-04), not just descriptive prose: (1) the four named orphan sub-categories (`MISSING_EPIC`/`MISSING_PARENT`/`DANGLING_LINK`/`FULLY_ORPHANED`, cited in all three docs) don't exist in code — only a single `isOrphan` boolean with one general impact statement. (2) The six-signal health-score formula's weights and even which signals are used (cited in all three docs) don't match `calculateHealthScore()` as shipped — different weights throughout, no `blocked_ratio` term in code, and the claimed "velocity_trend" signal is actually the latest sprint's raw completion rate. (3) Minor: "40+ field alias mappings" (disclosure + comparison table) — the actual `FIELD_ALIASES` table has 31 entries; doesn't undermine the architectural claim itself. |
