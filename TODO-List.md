...
| JIRA-10 | Update all related product docs | P1 | ✅ Done for JIRA-01–05/05b/04 (2026-06-20) | `product/SRS.md` Addendum G (FR-337–FR-340) + revision history row 4.9.3; `product/USE_CASES.md` UC-110; `product/TEST_CASES.md` §9.57 updated to reference UC-110/FRs; `product/RELEASE_NOTES.md` v4.5.0–v4.5.2 entries; `product/DEVELOPER_GUIDE.md` cross-ref. Schema-only/API-only earlier slices (JIRA-01–03) correctly had no UC/SCN/UJ per "no UC for vaporware" until JIRA-04 made the feature admin-reachable. **Still pending:** docs for JIRA-06/07/08/09(remaining)/manual-sync once those slices land. |
