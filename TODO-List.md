...
| CI-E2E-02 | Investigate: `mobile-dense-tables.spec.ts › roadmap Gantt and forecast table` — Gantt chart doesn't overflow horizontally at mobile viewport width | P2 | ❌ Not started | Surfaced by `CI-E2E-01`'s investigation (see that entry for full detail) — a genuine, pre-existing, unrelated E2E failure on `/roadmap`'s Gantt chart (`app/roadmap/page.tsx`'s `data-testid="gantt-scroll"` / `.ganttGrid` in `page.module.scss`), confirmed failing on `main` since at least 2026-08-03, independent of the Mediterranean theme branch. Not yet determined whether this is a real responsive-layout regression (the Gantt content genuinely doesn't need horizontal scroll at 390px with the current CI seed dataset — possibly too few/too-narrow-date-range roadmap items) or a stale test assumption. Needs its own investigation before a fix; not blocking the theme branch since it's demonstrably not caused by it. |
