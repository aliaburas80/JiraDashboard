...
| STYLE-06 | Refactor Tier 5 — remaining shared components | P1 | 🟡 In progress (2026-07-21) | Re-audited 2026-07-21: real scope was `src/components/explore/**` (`RelationCharts` 20, `WorkItemGraph` 16, `RelationDetailsTable` 4, `RelationLegend` 2, `RelationStatsCards` 1), `src/components/admin/**` (`DataRetentionSettings` 23, `AdminConsoleLayout` 13, `IssueTypeHierarchySettings` 5), `src/components/dc-shell/**` (`DCTopbar` 13, `DCActionBoard` 6, `DCKpiCard` 6, `DCPageSidebar` 4, `DCStatusChip` 2), plus ~18 more files at ≤7 warnings each (338 warnings, 62 files total repo-wide including the already-closed Tiers 1-4). Converted so far: `DataRetentionSettings.tsx` (23→0, new SCSS module, dark-admin `--dc-*` tokens moved out of inline `style`), `RelationCharts.tsx` (20→4, generic Donut/HBar/Legend chart primitives — remaining 4 are sanctioned `--donut-size`/`--donut-gradient`/`--bar-color`/`--dot-color` exceptions), `WorkItemGraph.tsx` (16→1, React-Flow node card — border/bg/box-shadow tone chain converted to `data-tone`/`data-focus`, remaining 1 is the per-issue-type admin-configurable color exception), `AdminConsoleLayout.tsx` (13→2, shared by 6 admin pages — each page's arbitrary stat-card tone/color destructured into `--tone-bg`/`--tone-color`/`--value-color` internally, no caller changes needed). `DCTopbar.tsx`/`DCActionBoard.tsx`/`DCPageSidebar.tsx` (13+6+4=23 warnings) turned out to be dead code — see `ORPHAN-05`, deleted instead of converted. Remaining: `IssueTypeHierarchySettings.tsx` (5), `DCKpiCard.tsx`/`DCStatusChip.tsx` (live, 6+2), `TrendChart.tsx` (5), `ThemeCustomizerPanel.tsx` (7), and the small-count tail. |
