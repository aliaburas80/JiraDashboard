...
| ID | Finding | Note |
|---|---|---|
| PRIV (retention) | `/privacy`'s stated retention windows aren't enforced by any scheduled process | Needs either: implement the scheduled job, or correct the policy copy to match reality — **a legal/product decision on which**, not purely an engineering one |
| PRIV (sub-processors) | `/privacy` omits Azure/GCP as disclosed sub-processors despite the code fully supporting both | Verify live deployment config first (this audit could not determine whether either is enabled for a live account) — if either is live, this becomes urgent; update policy language either way |
