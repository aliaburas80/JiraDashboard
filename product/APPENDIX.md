...
| **Dashboard Sub-Pages** | The 9 independently routed pages under `/dashboard/*` (introduced in v4.9.0, consolidated across three same-day-or-later passes: 2026-07-11 x2, 2026-07-12). Each page is a full Next.js App Router route sharing the `app/dashboard/layout.tsx` (DashboardTopbar + DashboardNavSidebar). Pages: priority-attention (incl. Smart Actions), key-metrics, data-quality (incl. delivery composition), trends (sprint + quarter, toggle), ownership, labels, epic-readiness, flow-health, coaching. `kanban-health`, `visual-analytics`, and `delivery-controls` were removed (100% duplicate content elsewhere); `actions` was merged into `priority-attention`; `sprint-status` and `quarter-statistics` were merged into `trends`; `delivery-composition` was merged into `data-quality`. All six merged-away routes redirect to their replacement. |
...
| **Delivery Composition** | A `/dashboard/data-quality` section (its own page until the 2026-07-12 merge). Shows work breakdown by status/health as a donut — Done, In Progress, At Risk, Critical, Backlog — with totals, orphan count, and story points alongside it. |
| **DC Shell** | The `src/components/dc-shell/` directory. Live contents: `navigation.ts` (`DC_NAV_GROUPS`), `DCKpiCard`, `DCStatusChip`. `DCTopbar`, `DCPageSidebar`, `DCActionBoard`, and `DeliveryClarityShell` were removed 2026-07-21 (`ORPHAN-05`) — dead code from an earlier design iteration, never mounted by any route, superseded by `DashboardTopbar`/`AppShell`. |
