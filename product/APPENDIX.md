...
| **Retrospective Page** | The `/retro` page. Provides a three-card landing: Fill in App (in-app form), Download Template (CSV), Upload Retro File (coming soon). Accessible to all roles via the Planning header group. |
| **Retro Insights** | The suggestions generated by `generateInsights(form)` after a retrospective is submitted. Evaluates sprint goal outcome, blocker count, missing action-item owners, missing due dates, and zero action items. |
