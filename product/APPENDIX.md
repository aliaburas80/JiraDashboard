...
| **Forecast Status** | The delivery health classification computed by `computeForecast()`: `complete` (done ≥ total), `insufficient_data` (no throughput), `on_track` (≤ 6 sprints remaining), `at_risk` (≤ 12), `off_track` (> 12). |
...
| **Retrospective Insights** | The structured `RetrospectiveInsight` produced by `generateRetrospectiveInsight()` — shared by the in-app form and uploaded files. Includes themes, ownership gaps, duplicate action items, next-sprint suggestions, ceremony recommendations, and a confidence rating. Replaces the original flat-string `generateInsights(form)`. |
| **Retrospective Theme** | One of 7 keyword-detected categories (process, communication, requirements, qa-release, dependency, technical, planning) found in a retrospective's free-text observations and blockers. Shown as a count-labelled chip on each insight panel. |
...
| **Dashboard Sub-Pages** | The 15 independently routed pages under `/dashboard/*` introduced in v4.9.0. Each page is a full Next.js App Router route sharing the `app/dashboard/layout.tsx` (DashboardTopbar + DashboardSidebarNav). Pages: summary, priority-attention, sprint-status, epic-readiness, labels, flow-health, key-metrics, kanban-health, visual-analytics, ownership, quarter-statistics, actions (Smart Actions), delivery-composition, delivery-controls, data-quality. |
...
| **Smart Actions Page** | The `/dashboard/actions` page. Generates AI-style actionable suggestions from the loaded metrics: critical blockers, stalled active items, capacity imbalance. Each action has a type (critical/warning/info), title, detail, suggested owner, and status (open/resolved). Filterable by action type. |
