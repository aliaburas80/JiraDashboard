...
| **Date** | 2026-08-16 |
| **Author** | Ali Abu Ras |
| **Status** | Approved — 4.15 is a doc-accuracy correction only (BR-123's Phase 1a slice re-merged to `main`, branch pointer stale); substantive content last changed in the 2026-06-28 catch-up pass: BR-118–BR-124 added, covering live Jira sync, admin-configurable issue hierarchy, role-based coaching insights, the retrospective insights engine, forecast engine v2, and the (design-only, not yet approved) multi-tenant organization initiative — see Revision Note below |
...
| 4.15 | 2026-08-16 | Ali Abu Ras | Doc-accuracy correction, no scope change — the Phase 1a slice described in BR-123 (schema + `scopedRepository` isolation core, additive-only) is no longer on an unmerged branch: it was re-merged to `main` via PR #40. BR-123 corrected below to describe this accurately — still a partial, not-yet-approved-as-a-whole slice, and still explicitly excluding the NOT NULL tightening step that caused the 2026-08-08 incident. See `TODO-List.md` §20a and `product/SRS.md` for the full history. |
...
- **Done** Coaching Insights Redesign & Encouragement Enhancements (v4.10.1) — scannable hero-banner layout, urgency-sorted tabs, severity trend vs. last saved snapshot, evidence chips linking to their source dashboard
