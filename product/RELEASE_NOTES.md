...
upload, verification, password-reset, landing, privacy policy, and terms of use pages, which
previously used a separate dark design and now match the rest of the app. If
...
## v4.17.0 — Request-Derived Email Links + "Forgot Your Password" (2026-07-04, P1)

**Scope:** Two fixes/features shipped together while closing out today's email-system work. First, a real user report — an old welcome email linking to `http://localhost:3000` instead of the production URL — traced to a genuine bug: `buildWelcomeEmail`/`buildVerificationEmail` links used `getAppConfig().appUrl`, sourced purely from `process.env.APP_URL`/`NEXT_PUBLIC_APP_URL`, silently falling back to a hardcoded `http://localhost:3000` when that env var is unset in a given environment (confirmed this was the case). Second, self-registered users had no way to recover a forgotten password (EP-013, previously unbuilt) — the login page had no such link at all.

- **`resolveRequestOrigin()` (`src/lib/url.ts`, new):** derives scheme+host from the actual incoming request (honouring `X-Forwarded-Proto`/`X-Forwarded-Host` from reverse proxies) instead of trusting a configuration value that can silently go stale or missing. Wired into both `POST /api/auth/register` and `POST /api/admin/user-add-requests/:id/accept`, replacing their `getAppConfig().appUrl` reads. Self-correcting — works in any environment (local, Render, a future custom domain) with zero configuration.
- **EP-013 "Forgot your password":** new `POST /api/auth/forgot-password` (public, 5/IP/hour rate limited, always returns the same generic response whether or not the email exists — enumeration-safe, mirroring `POST /api/auth/register`'s existing pattern) generates a 1-hour token (`User.passwordResetToken`/`passwordResetExpires`, deliberately shorter-lived than the 24h email-verification token since a leaked reset link grants immediate account takeover) and emails it via new `buildPasswordResetEmail()`. New `POST /api/auth/reset-password` consumes the token, sets the new password, and clears the token (single-use). New `/forgot-password` and `/reset-password` pages. Login page gained a "Forgot password?" link.
- **Verified**: `tsc`/ESLint clean; full suite 818/85 passing (18 new tests: 6 for `resolveRequestOrigin`, 12 for forgot/reset-password); manually exercised end-to-end against the real dev database — requested a reset, confirmed the token persisted, reset the password, confirmed the *old* password now fails login, the *new* one succeeds, and replaying the same token is rejected; screenshotted all new pages.
- **Documentation impact**: RELEASE_NOTES (this entry), SRS (Addendum L, FR-368–FR-372), TEST_CASES (§9.64), DEVELOPER_GUIDE (new section), `/help` (new FAQ entries).
...
- **Footer copyright corrected** — `© 2026 Ali Delivery Intelligence` → `© 2026 Ali Abu Ras · ali.aburas@deliveryclarity.app`, matching the in-app shell footer.
...
- **Demo email corrected**: the "Request a demo" CTA and footer Contact link both emailed `ali.aburas@deliveryclarity.app` (a character transposition); fixed to `ali.aburas@deliveryclarity.app` so demo requests actually reach the right inbox. Both spellings appear heavily across older vs. newer copyright headers in the repo — the correct, current address was confirmed with the owner before changing it.
...
- **HTML report export**: Lightning bolt brand mark (gradient square + SVG) added to header alongside "Delivery Report" title; footer updated to "Delivery Clarity v4.1 · Ali Abu Ras · ali.aburas@deliveryclarity.app"
...
- **Email consistency**: All UI and export footers now use `ali.aburas@deliveryclarity.app`
