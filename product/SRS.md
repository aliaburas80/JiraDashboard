...
| 4.33.3 | 2026-07-21 | Ali Abu Ras | P4 — ORPHAN-05, found while converting `src/components/dc-shell/DCTopbar.tsx` under `STYLE-06` (Tier 5 inline-style remediation). This repeated a finding already noted in this very file (§ "Note on the unused legacy shell", 4.32.x era) but never acted on: `DCTopbar.tsx`, `DeliveryClarityShell.tsx`, and (newly confirmed this pass) `DCActionBoard.tsx`/`DCPageSidebar.tsx` have zero live callers anywhere in `app/` — `DeliveryClarityShell` (the only importer of `DCTopbar`/`DCPageSidebar`) is itself never imported by anything, and `DCActionBoard` has no importer at all, not even `DeliveryClarityShell`. Confirmed via `grep` across `app/` and `src/` before deleting. Presented to the owner (same "convert vs. delete" fork as `ORPHAN-01`/`ORPHAN-02`); owner chose deletion. All 4 files removed (no dedicated tests existed for any of them). `src/components/dc-shell/navigation.ts` (`DC_NAV_GROUPS`), `DCKpiCard.tsx`, and `DCStatusChip.tsx` are unaffected — confirmed live consumers in `AppShell.tsx`, `DashboardTopbar.tsx`, `AdminNavSidebar.tsx`, `GlobalSearch.tsx`, `/data-quality`, `/flow-health`, `/release-readiness`, `/work-explorer`, and were not touched. Deleting this cluster also resolved 23 of the `react/forbid-dom-props` warnings STYLE-06/Tier 5 was tracking for these files, without spending conversion effort on dead code. `npx tsc --noEmit` clean; full suite unaffected, 113/113 suites (1082/1082 tests) passing; `npm run build` succeeds. |
| 4.34.0 | 2026-07-31 | Ali Abu Ras | P0 — `P0B-01`. Restored the registration "primary role" persona list from 5 back to its original 11 values (`src/lib/personas.ts` — Scrum Master, Agile Coach, Product Owner, Project Manager, Delivery Manager, Engineering Manager, Team Lead, Executive, Jira Administrator, Consultant, Other), reversing the 4.26.1 trim per explicit user instruction, to match the soft-launch master plan's persona spec (§4.1: "one primary role and zero or more secondary roles"). Added the secondary-role capability itself, new: `User.secondaryPersonas String[] @default([])` (additive migration `20260731000001_add_secondary_personas`, applied and verified via `prisma migrate deploy` against a live Postgres instance), a new `<fieldset>` checkbox group on `/register` excluding whichever role is chosen as primary, and server-side validation in `POST /api/auth/register` that dedupes, drops any value not in `PERSONAS`, and excludes the primary value — invalid/missing input is never a 400 for this field since it's optional and checkbox-driven in the real UI. `src/config/personaFocus.config.ts`'s `Record<Persona, PersonaFocusConfig>` mapped type forced 6 new focus-content entries (Agile Coach, Delivery Manager, Team Lead, Jira Administrator, Consultant, Other) to be authored before `tsc --noEmit` would pass — a structural completeness guard, not an optional nice-to-have. Also corrected `product/DEVELOPER_GUIDE.md`, which falsely claimed registration returns 403/is disabled by product policy in four places — directly contradicted by the live route and by `product/MANUAL_TESTS.md` §14's own documented walkthrough against production. Confirmed via code audit that this ticket's other stated scope (free signup, email verification, workspace/entitlement creation) was already fully implemented (EP-011/EP-012) — the tracker's "❌ Not started" status was stale. Persona remains confirmed cosmetic/analytics-only — `role: 'user'` is hardcoded server-side at registration regardless of submitted persona, and `persona`/`secondaryPersonas` are never read in any authorization check anywhere in `app/api` (verified via grep before this change). |
| 4.35.0 | 2026-08-01 | Ali Abu Ras | P0 — `P0B-02`. The core trial-entitlement state machine (EP-015, `src/lib/entitlement.ts`) was already live; closed the real gaps against master plan §4.1. **24h replacement upload**: new `Entitlement.replacementUsedAt` column (additive migration `20260801000001_add_entitlement_replacement`, applied and verified via `prisma migrate deploy` against a live Postgres instance), new `beginReplacementUpload`/`consumeReplacementUpload`/`revertReplacementUpload` functions operating on `replacementUsedAt` as the lock field — deliberately never touching `status`/`consumedAt`/`expiresAt`, so a replacement is anchored to the original successful analysis, not a fresh 30-day grant. **Closed a real bypass**: `app/api/upload/merge/route.ts` had zero entitlement gating — a consumed/expired/suspended non-admin user could fully bypass the trial through the multi-file merge flow; given the same wiring as the single-file route, plus two related gaps in the same file (no `ImportLog` row ever created — needed as entitlement's FK target — and the EP-011 email-verification gate was missing). **Expiry enforcement**: previously the only enforcement point anywhere in the app was the upload gate — a consumed/expired user's dashboard kept working indefinitely (no `middleware.ts` exists in this repo). Gated the one chokepoint every dashboard page funnels through, `GET /api/metrics/latest`, for non-admin `expired` users. **Found and fixed mid-pass, not original scope**: `app/page.tsx`'s "Try a sample first" ran through the exact same `/api/upload` path as a real upload, consuming the real trial entitlement — rewired to always use the network-free `processFileLocally()` pipeline regardless of `dataStorageMode`, since a genuinely free/non-destructive demo dataset is a prerequisite for the expiry design (not an adjacent bug). **Found via direct verification of the Plan agent's design, not the original research**: `src/lib/storage.ts`'s `loadMetricsWithSource()` fell back to a stale `localStorage` copy even when the server returned an authoritative block — since consuming a trial requires having loaded the dashboard at least once, this would have made the expiry check a no-op for nearly every real expired user. Fixed by special-casing the new `reason:'expired'` response to skip the local-fallback branch, and by extracting `redirectWithLoadError`'s signal-storage half into a standalone `src/lib/loadErrorSignal.ts` `setLoadErrorSignal()` so the explanation reaches `/` regardless of which of the 17 dashboard pages' two different redirect patterns (`redirectWithLoadError(router)` vs. a bare `router.replace('/')`) fires — confirmed both patterns exist via direct code reading, not assumed uniform. **Deliberately not built, with reasons traced in code**: mutating `Workspace.status` on expiry — `getWorkspaceForUser()`'s active-only filter and `getMetricsScopeKeyForUser()`'s fallback-to-a-different-scope-key behavior mean flipping it would silently redirect metrics reads/writes to a different storage key than real data was written under, worse than doing nothing; admin suspend/restore UI (`P0B-13`'s scope); gating `/api/snapshots`/`/api/trends`/`/api/imports/export` individually (small, explicitly-accepted residual); local-storage-mode uploads touching entitlement (FR-377, deliberate privacy design). **Testing:** `entitlement.test.ts` `TC-ENT-11`–`20` (10 new cases covering replacement detection/lock/revert/consume, plus closing pre-existing coverage gaps for `consumeEntitlement`/`createEntitlementForUser`/`restoreEntitlement`; one existing fixture, `TC-ENT-02`, fixed since its `consumedAt: new Date()` accidentally satisfied the new replacement window), new `uploadMergeEntitlement.test.ts` (eligible/blocked/admin-bypass/unverified-email/replacement-window cases), `uploadUserId.test.ts` (2 new replacement-window regression cases — the most important being that a *failed* replacement leaves `status` at `'consumed'`, not `'eligible'`, since flipping it would wrongly grant a fresh 30-day trial), new `metricsLatestEntitlement.test.ts`, `uploadMergeSignature.test.ts` mocks updated for the route's new `@/lib/entitlement`/`@/lib/prisma`/`@/lib/workspace` dependencies. **Verification gap disclosed, not worked around**: `loadMetricsWithSource()` and the `handleSampleData()` fix are client-side/React logic with no automated coverage — this repo has no jsdom or component-testing infrastructure anywhere (Node-only Jest, confirmed via `product/DEVELOPER_GUIDE.md`'s Tech Stack table and the identical gap already noted at FR-396) — verified via direct code reading and manual QA instead. |
| 4.36.0 | 2026-08-01 | Ali Abu Ras | P0 — `P0B-03`. Consent was previously just two flat, overwrite-once `User` fields (`termsAcceptedAt`/`termsVersion`, EP-014) with zero read/write call sites beyond registration — not an actual "record" despite the privacy policy already promising retained consent records, no in-app way to view consent status, and no "analytics choices" concept anywhere (confirmed no analytics/tracking system exists yet in this repo at all). `/terms`, `/privacy`, and the required registration consent checkbox were already fully built and are unchanged. Added a new append-only `Consent` table (one row per grant/withdrawal event, never updated in place — additive migration `20260801000002_add_consent`, applied and verified via `prisma migrate deploy` against a live Postgres instance) and `src/lib/consent.ts` (`recordConsent`/`getConsentStatus`), with `getConsentStatus` falling back to the legacy `User` fields for `terms_and_privacy` when no `Consent` row exists — true for every pre-existing account. Registration now writes a `terms_and_privacy` consent row inside the same transaction as user creation, alongside the unchanged legacy fields. New `GET`/`POST /api/consent` endpoints and a new Settings → Privacy tab (`PrivacyTab.tsx`) let a user view their terms-acceptance record and self-service toggle the only optional, togglable purpose — **analytics** — whose own UI copy discloses that no analytics collection exists yet. Required `terms_and_privacy` consent is deliberately not togglable through this endpoint; withdrawing it means requesting account deletion, correctly deferred to `P0B-04`. |
| 4.37.0 | 2026-08-02 | Ali Abu Ras | P0 — `P0B-04`. No self-service account deletion or export existed anywhere — `DELETE /api/admin/users` was admin-only and couldn't target its own account; `GET /api/imports/export` exported only ImportLog metadata. Deletion is a soft-then-hard two-phase design: `POST /api/account/delete` (password-verified) immediately sets `isActive: false` + a new `User.deletionRequestedAt` (additive migration `20260802000001_add_deletion_requested_at`) and destroys the session, reusing EP-010's existing `GET /api/auth/me` session-ejection check verbatim. A new daily GitHub Actions cron (`data-retention.yml` + `scripts/purge-expired-data.mjs`) hard-deletes accounts past a 7-day grace period — inside the Privacy Policy's published "within 30 days" SLA. 12 of 14 `User`-FK'd models already cascade-delete; `AuditEvent` (`SetNull`) and `Feedback`/`AppError` (no FK) are correct as-is. Admin reactivation now clears `deletionRequestedAt` so the grace window is cancelable. Export (`GET /api/account/export`) is a single downloadable JSON — profile, full `Consent` history, entitlement, workspace, import logs, snapshot content, and current live metrics — deliberately not CSV, avoiding the formula-injection surface entirely. New `src/lib/accountLifecycle.ts`. Verified, not assumed, that no uploaded originals are ever stored server-side (traced both upload routes — only computed metrics persist), so the "uploaded-original policy" line item is a documented finding, not a built purge job. Retention enforcement now covers `AuditEvent` (12mo) and `AppError` (90 days) — both previously unenforced — plus scheduling the existing manual-only ImportLog/DashboardSnapshot policy. Found and fixed in the same area: `app/api/admin/cleanup/route.ts` was reading retention settings from a disconnected legacy filesystem store instead of the DB-backed store the Settings UI actually saves to. See `product/RELEASE_NOTES.md` and the FR entry below for full detail. |
| 4.38.0 | 2026-08-02 | Ali Abu Ras | P0 — `P0B-09`. Tracker was badly stale — direct research found the feedback popup, its API, the full impact-level/status workflow, and a complete admin triage UI already built and tagged `P0B-09` in their own source comments. The one confirmed gap, verified via repo-wide grep (no `html2canvas`/`getDisplayMedia`/canvas-to-image utility anywhere): screenshot capture. Added `html2canvas` (new dependency, justified — no browser-native API fits; `getDisplayMedia()`'s OS permission prompt is disproportionately heavy for "attach a screenshot to a bug report"). Capture is opt-in and previewed, never automatic, inside the already-open feedback modal — resolves the "do NOT auto-attach Jira data" tension by construction, since nothing is captured or sent without an explicit user click and a visible preview first. New `Feedback.screenshotData String?` column (additive migration `20260802000002_add_feedback_screenshot`), stored as a plain blob column matching the existing `DashboardSnapshot.metricsJson` precedent rather than a new attachment table/service. Found and fixed before merge, not after: the admin feedback list endpoint would have shipped raw screenshot bytes for all 30 rows/page — fixed with an explicit field-stripping `select` + a lazily-fetched, one-at-a-time screenshot sub-route. |
| 4.39.0 | 2026-08-02 | Ali Abu Ras | P0 — `P0B-05`. A genuinely fresh subsystem, unlike every prior P0-B ticket this session — repo-wide grep confirmed zero prior art. Built the master plan §4.2 event taxonomy (8 domains, 30 named events) as a typed catalog and a client SDK (`src/lib/analytics/`) that assembles the full §6.2 event envelope (`event_id`, `schema_version`, pseudonymous `anonymous_id`/`session_id`, page/section/component, app version, role, browser/OS/device context, `properties`). Scope is deliberately envelope-construction only, not delivery — `P0B-06` (IndexedDB queue) and `P0B-07` (server ingestion) remain separate, dependent tickets; the SDK's transport is an injectable, currently-inert dev-only stub. Consent-gated by design against the analytics toggle P0B-03 already shipped — no event fires without it. Wired exactly one reference integration (`feedback_opened`/`feedback_submitted` in `FeedbackButton.tsx`), deliberately not a repo-wide instrumentation sweep. No new dependency. |
| 4.40.0 | 2026-08-03 | Ali Abu Ras | P0 — `P0B-06`. Another genuinely fresh subsystem — zero `indexedDB`/`sendBeacon`/`visibilitychange` usage anywhere before this. Built the master plan §4.7 batched-delivery queue: `src/lib/analytics/eventQueue.ts` (IndexedDB storage adapter) and `eventFlush.ts` (batching/retry/delivery orchestration), swapped in as `trackEvent()`'s default transport in place of P0B-05's inert dev-log stub. Flush triggers: 10s interval, 30-event threshold, `online` return, `visibilitychange`-hidden (via `sendBeacon`, which never deletes on send since it has no response — events are safely re-flushed and only then deleted on the next real ack), and immediately on any quality-domain event. Also ships a minimal, real `POST /api/events` (rate-limited, shape/taxonomy-validated, returns the master plan's exact `{accepted, rejected}` contract) resolving a genuine scope tension with `P0B-07` (deduplication/storage) — decided with the user before implementation — while persisting nothing itself. New devDependency `fake-indexeddb` (test-only). Retry/backoff was redesigned mid-implementation from a self-scheduling timer to a passive `Date.now()`-gated check after a real dangling-timer test-interference bug surfaced while writing the flush tests. |
| 4.41.0 | 2026-08-08 | Ali Abu Ras | Doc-accuracy correction, no code changed. Row 4.12.0's `feature/org-phase1-tenant-isolation` branch reference is stale — that branch is confirmed deleted (`TODO-List.md` §20a, found 2026-08-08) and its work genuinely lost, not just unmerged. It was rebuilt the same day as Phase 1a (`Organization` model, `scopedRepository.ts`, the `no-direct-org-scoped-prisma` ESLint rule, the same 12 `TC-ORG-01–12` tests, `backfillDefaultOrganization.ts`), applied as an additive migration to the live database, and backfilled against real production data with zero data loss. A subsequent Phase 1b attempt (tightening `organizationId` to `NOT NULL`, the design doc's required second migration step) correlated with a ~5–6 minute production 503 — reverted via `git revert` per `product/DEVELOPER_GUIDE.md` §12, service restored. Root cause not fully confirmed; follow-up static analysis (the failing routes touch no Prisma model) and external corroboration (a Hostinger platform maintenance window overlapping the outage) both point away from the migration itself, but this is not certain enough to retry without first deploying app code that supplies `organizationId` ahead of any constraint tightening. Current state: `main`'s `prisma/schema.prisma` has no `Organization` model and no `organizationId` fields (fully reverted); the complete rebuilt Phase 1a+1b work is preserved, unmerged, on `feature/org-phase1b-not-null-tighten` (pushed to `origin`). `TEST_CASES.md` §9.62 and `BRD.md` BR-123 corrected to match. |
| 4.42.0 | 2026-08-16 | Ali Abu Ras | Doc-accuracy correction, no code changed this session — the code itself (Phase 1a only, from the 4.41.0 rebuild) was re-merged to `main` via PR #40 (`14b108f`, `297b3f2`, merge commit `5e75ebb`). `main`'s `prisma/schema.prisma` now has the `Organization` model and nullable `organizationId` on `User`/`ImportLog`/`DashboardSnapshot`/`UserAddRequest`/`Notification`/`JiraConnection`; `src/server/tenancy/scopedRepository.ts`, the `no-direct-org-scoped-prisma` ESLint rule, `prisma/backfillDefaultOrganization.ts`, and the 12 `TC-ORG-01–12` tests are all on `main` — verified directly rather than assumed. Only the additive migration (`20260809000001_add_organization_nullable`) is present; Phase 1b's NOT NULL tightening remains deliberately excluded per the incident note above. Production (`https://deliveryclarity.app`) confirmed healthy (`200` after its normal auth redirect) both before and after this doc pass. `TODO-List.md` §20a, `BRD.md` BR-123, and `TEST_CASES.md` §9.62 corrected to match. |
...
- **Dashboard routed pages** — dashboard refactored into 9 independent Next.js App Router pages under `app/dashboard/[section]/`: priority-attention (incl. Smart Actions), key-metrics, data-quality (incl. delivery composition), trends (sprint + quarter, toggle), ownership (ownership & capacity), labels, epic-readiness, flow-health, coaching. `kanban-health`, `visual-analytics`, and `delivery-controls` were removed 2026-07-11 in a nav consolidation (100% duplicate content elsewhere); a same-day follow-up pass then merged `actions` into `priority-attention` and merged `sprint-status`+`quarter-statistics` into `trends`; a further pass on 2026-07-12 merged `delivery-composition` into `data-quality`. All six merged-away routes redirect to their replacement.
...
- **DC shell component library** — `src/components/dc-shell/`: `DCKpiCard`, `DCStatusChip` (`DCTopbar`/`DCPageSidebar`/`DCActionBoard`/`DeliveryClarityShell` removed 2026-07-21, ORPHAN-05 — dead code, never mounted)
...
**FR-353 (P1 — Done, 2026-06-26; superseded 2026-07-12, see Addendum AD):** ~~The Coaching Insights hero banner MUST display a severity-trend indicator (improved/worsened/unchanged) whenever the requesting user has at least 2 saved `DashboardSnapshot` records...~~ The new Team Role View (FR-411) has no hero banner, snapshot-trend indicator, or category tabs. `coachingTrend.service.ts` is no longer called by any page — see the orphan note in Addendum AD.

**FR-354 (P1 — Done, 2026-06-26; superseded 2026-07-12, see Addendum AD):** ~~Each coaching evidence chip's `metricKey` MUST be resolved against a static metric-family-to-route lookup (`resolveEvidenceRoute()`, `src/lib/coachingEvidenceLink.ts`)...~~ The new Team Role View has no evidence chips. `coachingEvidenceLink.ts` is no longer called by any page — see the orphan note in Addendum AD.
...
## Addendum V — Investigation: "Sample Data Keeps Showing, Real Upload Never Appears" (2026-07-08)

**User report:** "sample data keep show all of the time and whenever I upload new csv/excel the sample data keeps show and what I uploaded never been shown." Investigated as a possible fourth manifestation of the same underlying class of bug as Addenda R/S/T, but this one could **not** be reproduced after exhaustive testing of every layer involved — documented here in full rather than silently closed, since the user's underlying symptom may still be occurring for a reason not yet identified.

**What was tested and confirmed working:**
1. **Live server-side (cloud/App storage) path**, tested end-to-end against the real dev database using a disposable throwaway test account (registered, verified, and deleted again within this session — never touched the user's real account or data): uploaded a 2-issue file, confirmed `GET /api/metrics/latest` returned it correctly; uploaded a second, different 3-issue file over it, confirmed `GET /api/metrics/latest` immediately and consistently (5 repeated checks) returned the *second* file's data, never the first. The full parse → validate → calculate → write → read round trip is provably correct right now.
2. **Client-side localStorage save/overwrite logic** (`src/lib/storage.ts`), which is the only persistence layer for local-storage-mode accounts (the reporting user's account is confirmed, via a read-only DB query, to be in `local` mode) — added TC-DATAISO-16/17, which specifically simulate the reported scenario: old cached data already present (standing in for a prior "Try a sample first" click), then a real upload saved on top of it, both via the direct `saveMetrics()` → `loadMetrics()` path and the full `clearMetrics()` → `saveMetrics()` → `loadMetricsWithSource()` sequence `handleProceed()` actually uses. Both prove the new data fully replaces the old, even under artificial network latency on the ownership-tag fetch.
3. **`parseJiraFile`** (`src/services/jira/parser.ts`) has no module-level or shared state between calls — ruled out a stateful XLSX-library-caching theory.

**What could not be ruled out** (no browser-automation tool and no access to the user's actual file this session):
- Whether the user's specific export is missing one of the four essential columns (`Issue Key`, `Issue Type`, `Summary`, `Status`) or uses a column-name variant not in `FIELD_ALIASES`, which would either hard-fail validation (visible red error banner) or — more subtly — leave the mapping-preview screen's "Proceed to Dashboard" auto-countdown disabled (`missingEssential.length > 0` blocks it entirely, replacing the action row with only an "Upload a different file" button) — meaning the user's real data would never reach `saveMetrics()` at all, and old cached data would simply persist untouched. This is consistent with the report but unverified without their file.
- Whether the user was testing a deployed URL that had not yet been rebuilt from `main` at the time, or a browser tab holding an old cached JS bundle from before today's ownership-tag-race fix (Addendum T) — asked the user directly; response was "both" (localhost and a deployed URL), which argues against pure deployment staleness but doesn't rule out a stale browser tab/bundle on either.

**Hardening applied despite not finding the root cause** — found and fixed one more occurrence of the exact bug class fixed in Addendum T, in a different file, on the reasoning that it's cheap, safe, and closes a real (if narrower) gap even though it wasn't proven to be the cause here:
- `src/lib/localImportHistory.ts`'s `addLocalImport()` also called `tagCurrentOwner()` without awaiting it (identical fire-and-forget pattern to `saveMetrics()` before Addendum T). This function only writes the *upload history list* (`dc_local_import_history_v1`, shown on the profile page), not the dashboard metrics themselves (`dc_metrics_v2`) — so on its own it would not explain the reported symptom — but it's the same unsafe pattern and is now fixed the same way: `addLocalImport()` is `async` and awaits the tag write; its one call site (`app/page.tsx`'s `handleFile`) now awaits it.

**Verification:** 4 new tests (TC-DATAISO-16, TC-DATAISO-17, TC-LIH-10, plus TC-LIH-01–09 updated for the new `async` signature) plus the 5 manual live-server upload checks described above. Full suite 109 suites / 1004 tests passing (up from 109/1001). `npx tsc --noEmit`, `npx eslint` (touched files clean), `npx next build` all clean.

**Recommendation left for the user:** since this could not be reproduced through code inspection or the layers testable without a browser, the fastest way to pin it down is to watch it happen once — specifically, whether the "Column Mapping Preview" screen (a white card with a score circle and column list) appears after selecting the real file, and whether it shows a "Required columns missing" red banner with no working "Proceed" button. If so, the fix is either broadening `FIELD_ALIASES` for their export's exact column names or re-exporting with the four essential columns included — not a code defect in the save/load pipeline, which is now proven correct at every layer that could be tested.

## Addendum W — Navigation Duplication Cleanup: One Registry Instead of Five (2026-07-09)

**Reported problem, with screenshots:** duplicate/confusing menu entries across the topbar Reference menu, the user avatar dropdown, the Admin sidebar, and the Settings nested menu — "User Management" appeared as both a top-level Admin sidebar item and nested under Settings; "Settings" appeared in both the user dropdown and the Admin sidebar; Members/User Management/Member Requests overlapped conceptually.

**Audit findings — five independently hand-maintained navigation lists, not two:**
1. `src/components/dc-shell/navigation.ts`'s `DC_NAV_GROUPS` 'administration' group (6 items) — fed the topbar dropdown and global search.
2. `AdminNavSidebar.tsx`'s local `NAV_SECTIONS` (9 items across 3 sections: Activity/Observability/Configure) — the actual rendered Admin sidebar. Overlapped 6 of the 6 items in (1) under different ids/icons, plus had 3 items ((1) never had at all: Audit Events, User Feedback, System Errors — meaning global search couldn't find them).
3. `AdminNavSidebar.tsx`'s local `SETTINGS_SUB_ITEMS`/`PERSONA_PREVIEW_SUB_ITEM` (12 items) — the nested Settings sub-nav shown in the screenshots.
4. `src/lib/adminConsole.ts`'s `ADMIN_TABS` (12 items) — an independently-maintained, near-identical duplicate of (3), used for the settings page's own tab content/title/description.
5. `src/components/auth/UserMenu.tsx` — hardcoded 4 admin-only links (User Management, Import Logs, Security, Settings) directly duplicating destinations already in (1)/(2), shown to every admin on every page via the avatar dropdown.

Also found, while auditing: `app/admin/settings/page.tsx` had TWO separate, functionally-overlapping user-management implementations — the standalone `/admin/users` page and a `UserManagementSettings` component rendered as the (until now, nav-visible) "users" tab inside `/admin/settings`, both hitting `GET/POST /api/admin/users`. Deduplicating the *menu entry* pointing at the settings-tab version is in scope for this fix (per explicit instruction: "Preferred: keep User Management as its own Admin sidebar item, and keep Member Requests inside Settings"); merging or removing the underlying duplicate *feature* implementation is out of scope (no redesign requested) and is flagged here for a future task. Also found `AdminConsoleLayout.tsx`'s `ADMINISTRATION_NAV` export and its `navItems` prop were completely dead — accepted by the component but never rendered anywhere in its JSX — a sixth phantom list contributing zero visible duplication but real maintenance confusion.

**Fix — consolidated to two sources of truth, not one, because the two remaining sources serve genuinely different, already-independently-tested concerns:**
1. `src/components/dc-shell/navigation.ts` — `DC_NAV_GROUPS`'s 'administration' group is now the single source for every admin *destination* (expanded from 6 to 9 items, merging in Audit Events/User Feedback/System Errors so they're finally searchable). `DCShellNavItem` gained an optional `section?: string` field (Activity/Observability/Configure) consumed only by the new `getAdminNavSections(role, isSuperAdmin)` helper, which `AdminNavSidebar.tsx` now calls instead of hardcoding `NAV_SECTIONS` — the topbar dropdown and global search continue rendering the same group flat, ignoring `section`. Removed the dead `DC_ADMIN_NAV_ITEMS` export (confirmed zero consumers) and the dead `ADMINISTRATION_NAV`/`navItems` prop from `AdminConsoleLayout.tsx` (and the now-pointless `settingsNavItems` construction that fed it from `app/admin/settings/page.tsx`).
2. `src/lib/adminConsole.ts` — `ADMIN_TABS` remains the single source for Settings *tabs* (already richer — has descriptions and a `superAdminOnly` flag — and already consumed by the settings page for tab content/title, so this direction of consolidation touches less call-site code than the reverse). `AdminNavSidebar.tsx`'s Settings sub-nav is now *derived* from `ADMIN_TABS` (filtered for a new `hiddenFromNav` flag and `superAdminOnly`) instead of maintaining a second, separate `SETTINGS_SUB_ITEMS` array that had already drifted (e.g. different icon choices) from it. `ADMIN_TABS`'s `users` entry is marked `hiddenFromNav: true` — per the explicit "pick one location" instruction, User Management stays as its own Admin sidebar item (`/admin/users`) and is no longer offered a second time under Settings; the tab itself is untouched and still directly reachable (`/admin/settings?tab=users`), only the duplicate *menu entry* is removed. `app/admin/settings/page.tsx`'s default tab (when no `?tab=` is present) changed from `'users'` to `'requests'`, since Member Requests is now the first tab still offered in nav.
3. `src/components/auth/UserMenu.tsx` — removed all 4 admin-only links. The dropdown now contains only account-level actions: "My Settings" (`/profile`) and a new "Security" link (`/profile?tab=security`) — added `?tab=` deep-link support to `app/profile/page.tsx` (mirrors the existing pattern already used by `/admin/settings`) so this link lands directly on the Security tab instead of requiring an extra click. Admin/system destinations are reached via the Admin sidebar or the topbar's Administration dropdown, never via the user's personal-account menu.
4. Global search required no code change — it already consumes `getNavGroupsForRole()` → the same `DC_NAV_GROUPS`, so expanding the 'administration' group in place automatically surfaced the 3 previously-unsearchable items and stayed duplicate-free by construction (verified by new tests, not just by reasoning).

**Active-state correctness (requirement 4):** unchanged behaviorally — `AdminNavSidebar`'s parent-highlight logic (`item.href === '/admin/settings' ? onSettings : ...`) and child-tab-highlight logic (`activeTab === sub.tab`) operate on the exact same `href`/`tab` values as before, just sourced from the consolidated registries instead of local constants. One pre-existing, unchanged-by-this-fix quirk noted for the record: visiting the bare default tab's *explicit* query form (e.g. `/admin/settings?tab=requests`) doesn't highlight its own sub-nav entry (which uses `tab: ''`/bare-URL matching) — this quirk existed identically before this change (with `users` as the bare-URL entry instead of `requests`) and is not a regression.

**Testing (requirement 6):** 12 new tests.
- `navGroupsForRole.test.ts` (TC-NAV-11–17): no duplicate `id` or `href` anywhere across `DC_NAV_GROUPS`; admin's Administration group has exactly 9 unique items; `getAdminNavSections` groups correctly into the 3 labelled sections with no cross-section duplicates; a non-admin role gets zero admin sections; "User Management" (by title) appears exactly once in the whole registry, pointing at `/admin/users`; the Reference group contains no `/admin/*` destinations.
- `adminSettingsConsole.test.ts` (TC-NAV-18, 5 sub-tests): `users` is `hiddenFromNav` but remains a fully valid, directly-linkable tab (functionality preserved — only the duplicate menu entry removed); the nav-derivation filter (mirroring what `AdminNavSidebar.tsx` actually computes) excludes `users` for both a regular admin and a super-admin, while correctly including/excluding `personaPreview` by super-admin status; `requests` is confirmed as the first tab still offered in nav.

**Verification:** `npx tsc --noEmit` clean. Full suite 109 suites / 1020 tests passing (up from 109/1004), no regressions. `npx eslint` — zero new warnings on every touched file (confirmed by diffing warning counts against the pre-change baseline for `AdminConsoleLayout.tsx`, the one file with pre-existing unrelated inline-style warnings). `npx next build` clean — spot-checked that every `href` used in the consolidated registry (`/admin/audit`, `/admin/logs`, `/admin/feedback`, `/admin/system-errors`, `/admin/diagnostics`, `/admin/security`, `/admin/users`, `/admin/settings`, `/admin/theme`) corresponds to a real prerendered route in the build output — none are typos pointing at dead routes. **Not independently verified**: the actual rendered UI in a browser (that the Admin sidebar visually still shows the same 3 sections in the same order, that the user dropdown no longer shows the 4 removed admin links, that clicking "Security" from the user menu actually lands on the Security tab) — no browser-automation tool was available this session; the underlying data/logic driving every one of these surfaces is unit-tested, but the final rendered pixels were not visually confirmed.

## Addendum X — Complete User Management Dedup + Theme Page Simplified to Branding Only (2026-07-09)

**Follow-up to Addendum W**, same day, direct user instruction with a screenshot: "we still have 2 user managment, keep the one out and remove one iside config." Addendum W had hidden the Settings-tab "User Management" from nav (`hiddenFromNav: true`) but deliberately left the underlying `UserManagementSettings` tab implementation in place, flagged as a separate duplicate *feature* (not just a duplicate menu entry) out of scope pending explicit approval. That approval is this instruction — removed entirely, not just hidden.

**FR-1 — Removed the Settings "users" tab implementation, not just its nav entry:**
1. `src/lib/adminConsole.ts` — `'users'` removed from the `Tab` type union entirely (compile-time guarantee it can never be selected again, not just a runtime filter) and from `ADMIN_TABS`. `buildSettingsStats`'s `'users'` case (and its now-unused `userSummary` parameter) removed.
2. `app/admin/settings/page.tsx` — deleted the ~420-line `UserManagementSettings` component (table, inline role/status editing, bulk actions, delete-confirmation dialog — a full second implementation of what `/admin/users/page.tsx` already does against the same `GET/POST /api/admin/users` endpoints) along with its render branch, the `userSummary` state and the `fetch('/api/admin/users')` call that only existed to populate it, and `handleUsersChange`. Pruned now-unused imports (`useCallback`, `useRef`, `ManagedUser`, `roleOptionsFor`, `matchesUserFilter`, `ASSIGNABLE_ROLES`, `roleLabel`, `AppRole`) — confirmed each had zero remaining references before removing, since some of these names are still legitimately used elsewhere in the same file for other tabs (e.g. `SvgIcon`, `PasswordInput`, `ConfirmDeleteDialog` all remain, used by `CloudStorageSettings`). `roleOptionsFor`/`matchesUserFilter`/`ManagedUser` remain exported from `adminConsole.ts` since `/admin/users/page.tsx` — now the sole user-management implementation — still needs them.
3. Result: User Management exists in exactly one place, `/admin/users`, reachable from exactly one nav entry, the Admin sidebar's Configure section.

**FR-2 — Theme & Branding page cut down to Branding only:** separate, explicitly-scoped request: "I dont believe this is helpful page, since its useless, remove templates, keep things really user can use, and app benefit." Clarified scope with the user before touching anything (a full removal of theme customization is a one-way UX decision worth confirming, not guessing at): confirmed "Branding only" — remove the 4-preset colour-palette gallery (decorative mini-dashboard preview cards from "the UIUXTemplate design system," per the page's own prior description text), the font-size/corner-radius preset pickers, the font-stack reference block, and the 13-swatch "Active Token Reference" technical table; keep app name, favicon, and logo upload, since those have real business value (white-labeling) and are the only parts an admin would actually use.
1. `app/admin/theme/page.tsx` — rewritten from 480 lines to ~180: removed `PaletteCard`, `paletteClass`, `TOKEN_SWATCH_CLASS`, all `PALETTE_PRESETS`/`FONT_SIZE_PRESETS`/`RADIUS_PRESETS`/`applyThemeCustom`/`loadThemeCustom`/`saveThemeCustom`/`DEFAULT_THEME`/`ThemeCustom` imports and the state/handlers built on them (`updatePalette`, `updateRadius`, `updateFontSize`). Kept `LogoUpload`, `branding` state, `updateBranding`, `handleSave`/`handleReset` (now branding-only). Stat cards changed from palette/font/radius/app-name to app-name/logo/favicon.
2. `app/admin/theme/page.module.scss` — rewritten from 802 lines to ~230: removed every class only used by the deleted sections (palette cards and their 5 dark-mode CSS-custom-property variant blocks, mini KPI/nav preview strips, preset buttons, font-stack reference, token grid/swatches — roughly 570 lines of now-dead CSS). Kept only classes the simplified page actually renders (`section`, `sectionTitle`, `sectionDesc`, `brandingGrid`, `fieldLabel`, `textInput`, `inputHint`, logo-upload classes, action-button classes).
3. `src/components/dc-shell/navigation.ts` — the `admin-theme` nav entry (Admin sidebar Configure section, topbar dropdown, global search) relabeled from "Theme & Branding" / "Palette, logo, app name" / `palette` icon to "Branding" / "Logo, favicon & app name" / `tag` icon, so the label matches what the page now actually does — avoiding creating a new, smaller instance of exactly the labeling-mismatch problem Addendum W just fixed.
4. **Deliberately left untouched:** `src/lib/themeCustomizer.ts` (the underlying apply/load/save engine) and `src/components/layout/AppShell.tsx`'s `initThemeCustom()` call, which every page depends on to actually apply whatever theme is in `localStorage` (or `DEFAULT_THEME` if nothing was ever saved). Removing the *admin UI* for changing palette/font/radius doesn't require touching the engine that applies them — a browser that had previously saved a custom palette (before this change shipped) keeps rendering it exactly as before; there's simply no longer an admin control to change it, since in practice nothing but the page just removed ever wrote anything other than the default. Also confirmed out of scope and left alone: `src/components/ui/ThemeCustomizerPanel.tsx`, a separate, already-dead (unmounted anywhere) per-user theme picker unrelated to this admin page — noted for a future cleanup pass, not silently deleted as part of an unrelated task.

**Testing:** rewrote `adminSettingsConsole.test.ts`'s TC-AC-01–03 to use `'requests'`/`'cloud'` instead of the now-nonexistent `'users'` tab id (TS itself now rejects `'users'` as a `Tab` at compile time — confirmed via `tsc`, not just a runtime assertion); kept `roleOptionsFor`/`matchesUserFilter` tests since those helpers remain live, now documented as belonging to `/admin/users`. Rewrote TC-NAV-18 (5 tests → 3): `ADMIN_TABS` contains no `'users'` entry at all; `requests` is first; `personaPreview` filtering by `superAdminOnly` still works independently of the users-tab removal. No new tests needed for the theme page — it has no domain logic left, only branding state already implicitly covered by existing manual-verification guidance (no automated coverage previously existed for this page either).

**Verification:** `npx tsc --noEmit` clean. Full suite 109 suites / 1016 tests passing (down from 1020 — net fewer test cases after consolidating the users-tab tests into fewer, more accurate ones; no coverage lost, since the removed cases tested code that no longer exists). `npx eslint` and `npx stylelint` both clean with zero warnings on every touched file, including the fully-rewritten SCSS module. `npx next build` clean — bundle size dropped as expected from removing dead code: `/admin/theme` 10.4 kB → 7.54 kB, `/admin/settings` 36.1 kB → 34.9 kB. **Not independently verified**: the actual rendered `/admin/theme` and `/admin/settings` pages in a browser — no browser-automation tool was available this session.

## Addendum Y — Settings Sub-Nav Un-Nested Into Its Own Flat Section (2026-07-09)

**Follow-up, same day, with screenshot:** "need to bring sub-menu out and setting as lable." The Admin sidebar's Settings sub-nav (Member Requests, App Config, Privacy & Retention, Health Thresholds, Orphan Rules, Issue Type Hierarchy, Backup & Restore, Cloud Storage, Jira Integration, Browser Data, Persona Preview) was previously rendered indented beneath a clickable "Settings" link, and only appeared at all once already on `/admin/settings` — a collapsed/hidden-until-active pattern. Requested change: promote those items to their own always-visible, non-indented section, with "Settings" becoming a plain section label (matching "Activity" / "Observability" / "Configure") rather than a clickable nav item.

**Fix, `src/components/admin/AdminNavSidebar.tsx`:**
1. The Configure section's rendered items now exclude `admin-settings` (filtered out via `.filter(item => item.id !== 'admin-settings')` — the shared registry in `navigation.ts` is untouched, since the topbar dropdown and global search still legitimately offer "Settings" as one destination; this filter is local to the sidebar's own rendering).
2. Added a new, unconditionally-rendered `Settings` section (same `.navSection`/`.navSectionLabel` markup as the other three sections) containing the 11 settings tabs as flat, top-level-styled links (`.navItem`, same visual weight as "Audit Events" or "User Management" — not the smaller indented sub-item style used before). Each links to `/admin/settings?tab=<id>` (or bare `/admin/settings` for the first tab, Member Requests, unchanged from before); active-state highlighting is unchanged in logic (`onSettings && activeTab matches`), just no longer gated behind first rendering a parent item.
3. `renderNavItem`'s active-state branch for `href === '/admin/settings'` was dead code after step 1 (that item is never passed to it anymore) — simplified back to the plain path-match check.
4. Removed the now-unused `.subNav`/`.subNavItem`/`.subNavItemActive`/`.subNavIcon` classes from `AdminNavSidebar.module.scss` (indentation/small-text styling for the old nested pattern, no longer rendered).

**Scope note:** this only affects the Admin sidebar's own rendering. `DC_NAV_GROUPS`' `admin-settings` entry (topbar dropdown, global search) is unchanged — "Settings" remains a single, valid destination there, since those surfaces don't have room for (and weren't the subject of) an always-expanded tab list.

**Verification:** `npx tsc --noEmit` clean. Full suite 109 suites / 1016 tests passing, unchanged (no test previously covered this component — this repo's Jest cannot render `.tsx` components; verified no test file references `AdminNavSidebar` or the removed `subNav*` classes before removing them). `npx eslint` and `npx stylelint` both clean, zero warnings. `npx next build` clean. **Not independently verified**: the actual rendered sidebar in a browser — no browser-automation tool was available this session.

## Addendum Z — Product Tour Revived: Multi-Page, Real Anchors, SCSS-Compliant (2026-07-09)

**User question:** "i remamber there were a user guid or toure, what happend to it?" Investigated before answering: `src/components/tour/ProductTour.tsx` and `src/lib/tour.ts` were fully built (8-step highlight-ring-and-popover walkthrough, keyboard nav, skip/replay, localStorage-persisted dismiss/complete state) but **never mounted anywhere** — grepped every `.tsx` file for `ProductTour` and found zero consumers. It was also **stale**: every step's `targetId` (`section-overview`, `section-attention`, `dashboard-sticky-bar`, etc.) referenced DOM ids from when the dashboard was one single page with named section anchors; none of those ids existed anywhere in the current tree, since the dashboard was later split into ~16 separate routed pages (`DC_NAV_GROUPS`' `analytics` group in `navigation.ts`). Worse, `/help` still actively described the feature as working, telling users to click a "Take a tour" button on `/summary` and a "Tour" button on `/dashboard`'s header — neither existed. Presented these findings and asked the user to choose revive / remove / docs-only; chose **revive**.

**Design constraint discovered mid-investigation:** the dashboard's routes span two different layout systems — `/summary` uses a per-page `AppShell` wrapper (fully unmounts/remounts on every navigation, per its own existing code comment), while `/dashboard/*` uses a persistent Next.js `layout.tsx`. A tour spanning both would lose all state (active step) the moment it crossed that boundary if mounted inside either. Fixed by mounting `<ProductTour />` once at the true root (`app/layout.tsx`, alongside the other global singletons — `FeedbackButton`, `GlobalErrorHandler`, `EmailVerificationBanner`) — the one place guaranteed to survive every internal navigation in the app.

**Made it genuinely multi-page**, not just re-anchored to one page:
1. `TourStep` gained an optional `route`. `ProductTour.tsx` added a `usePathname()`-driven effect: when the active step's `route` differs from the current page, `router.push(route)` fires. No "wait for navigation to finish" handshake was needed — the existing `requestAnimationFrame` target-tracking loop (already running continuously to handle scroll/resize) simply keeps polling and picks up the new page's `targetId` the moment it mounts.
2. Remapped all 8 steps to real, current destinations: Welcome (center, wherever started) → Health Score & Key Metrics (`/summary`, the KPI grid) → Dashboard Navigation (`/dashboard/priority-attention`, the sidebar) → Priority Attention (same page, the page header) → Smart Recommendations (`/dashboard/actions`) → Sprint Throughput (`/dashboard/sprint-status`) → Work Item Explorer (CTA navigates to `/explore`, ends the tour — unchanged from the original design) → Done (center).
3. Added real anchor ids for the tour to find: `PageHeader` (`DashboardPageShell.tsx`) gained an optional `id` prop, passed as `tour-priority-attention` / `tour-recommendations` / `tour-sprint` from the three respective pages; `tour-kpi-grid` added directly to `/summary`'s KPI grid div; `dashboard-nav-sidebar` added to a small, fixed-height block near the *top* of `DashboardNavSidebar` — deliberately not the full-height `<aside>`, since the popover's positioning logic only offsets above/below a target (no true "beside" placement) and a full-viewport-height target would push the popover off-screen.
4. Added the two trigger points the (now-corrected) `/help` text describes: a "Take a tour" `ToolbarButton` on `/summary`, and a "Tour" button in `DashboardTopbar.tsx`'s right rail (visible on every `/dashboard/*` page, a slight improvement over the original single-header-only trigger, made possible by the topbar being the persistent-layout component). Both simply `window.dispatchEvent(new CustomEvent('dc:start-tour'))` — the mechanism `ProductTour.tsx` already listened for.

**Brought it into CLAUDE.md compliance while reviving it:** the entire original component was inline-style JSX (13 `react/forbid-dom-props` warnings) — dormant and therefore harmless while unmounted, but now genuinely shipping to users for the first time. Converted fully to a new `ProductTour.module.scss`: static values (colors, radii, shadows, typography, the two `@keyframes`) moved to SCSS; only the truly runtime-computed values (highlight-ring position/size from the live target rect, popover position from the same) remain as the documented `--`-prefixed CSS-custom-property exception (CLAUDE.md §14.2) — down to 2 justified warnings from 13 raw violations. Removed the manual `InjectStyles` component (a `useEffect` that hand-built and appended a `<style>` tag for the keyframes) — no longer needed, since CSS Modules scope `@keyframes` the same way they scope class names (matching the existing `dc-sync-spin` pattern already used in `DashboardTopbar.module.scss`).

**Testing:** rewrote the pre-existing but self-referentially-useless `productTour.test.ts` TC-PT-08 — it used to check `targetId` values against a hardcoded "known ids" list that was itself the stale list, so it always passed without proving anything real about the live app. Replaced with a source-tree grep (`idExistsInSource`) that confirms every `targetId` a step references is actually rendered by some `.tsx` file under `app/`/`src/components/` today. Added TC-PT-09/10, which confirm every `route`/`href` a step references corresponds to a real `app/<route>/page.tsx`. **Proved these tests are meaningful, not vacuous**, by the same method used earlier this session for other regression tests: temporarily renamed one real anchor id to a wrong value, reran the suite, confirmed TC-PT-08 failed with the expected assertion, then restored the correct id and confirmed it passed again.

**Verification:** `npx tsc --noEmit` clean. Full suite 109 suites / 1018 tests passing (up from 1016), including the 3 rewritten/added `productTour.test.ts` cases proven non-vacuous as described above. `npx eslint` — every file touched clean except `ProductTour.tsx`'s 2 fully-documented, justified CSS-custom-property-exception warnings (down from 13 raw violations) and pre-existing unrelated warnings on `/help`/`/developer` (part of the tracked §60 backlog, not touched by these changes' actual line ranges). `npx stylelint` clean on the new SCSS module. `npx next build` clean. **Not independently verified**: the actual rendered, multi-page tour experience in a browser — clicking through all 8 steps, confirming the highlight ring correctly tracks each target across real page navigations, confirming the popover never renders off-screen — no browser-automation tool was available this session. The routing/anchor-existence logic is proven correct by the tests above; the final visual polish (exact popover offsets, ring padding) was not visually confirmed.

## Addendum AA — Product Tour Rebuilt: Per-Page Instead of Cross-Page, ~49 Routes Covered (2026-07-09)

**Same-day follow-up to Addendum Z.** User request (verbatim): "I want to cover the app in the toure so the user know what to do, to reduce the amount of the job, each page has its own tour, so when enter any page toure there for this page to explan what user see." This is a different design from Addendum Z's single 8-step tour that hopped across 4 pages — the user wants every page to explain itself, on its own, without navigating anywhere. Clarified three scope decisions via `AskUserQuestion` before building: (1) cover **all major nav routes**, not just the dashboard, (2) **manual trigger only** (no auto-start popping up unprompted on first visit), (3) **replace** the Addendum Z cross-page tour entirely rather than keep both. User picked the recommended option on all three.

**Architecture change:** `TOUR_STEPS` (one flat array with `route`/`href`/`ctaLabel` fields driving cross-page navigation) replaced with `PAGE_TOURS: Record<pathname, TourStep[]>` in `src/lib/tour.ts` — each route owns its own short (1–2 step), self-contained tour describing only what's on that page. `getPageTour(pathname)` is the single lookup function. `ProductTour.tsx` dropped its `router.push`-on-step-change effect entirely (no longer needed — a tour never leaves the page it started on) and gained a `useEffect` that closes the tour on every pathname change, since a new page always has its own separate tour rather than a continuation. The old localStorage dismiss/complete tracking (`isTourDismissed`, `isTourCompleted`, `dismissTour`, `completeTour`, `resetTour`) was removed outright — with auto-start gone (manual trigger only, per the scope decision above), there was nothing left for that state to gate, and keeping it would have been dead code.

**New single trigger mechanism — `PageTourButton`:** replaces both of Addendum Z's ad-hoc buttons ("Take a tour" on `/summary`, "Tour" in `DashboardTopbar.tsx`) with one floating pill (`src/components/tour/PageTourButton.tsx`), mounted once at the root layout next to `<ProductTour />`. It reads `getPageTour(usePathname())` and renders nothing on a route with no registered tour — so no page ever needs its own bespoke trigger button again; adding a new page's tour to `PAGE_TOURS` is the only step required to make the button appear there. Positioned directly above `FeedbackButton`'s existing floating trigger (`inset-block-end: 76px` vs. `FeedbackButton`'s `24px`, same `inset-inline-end: 24px`) so the two stack without overlapping.

**Coverage — 49 routes, real anchors, no fabricated copy:** every route in `DC_NAV_GROUPS` / `getAdminNavSections()` (analytics, delivery, planning, data, administration, reference groups) plus `/profile` (linked from the user-menu dropdown but not in the nav registry). Anchor and copy work was parallelized across 4 background agents, each scoped to a distinct, non-overlapping file set, then every returned title/subtitle/description string was used verbatim or near-verbatim as the tour copy — chosen specifically to avoid fabricating page descriptions from guesswork:
- **16 dashboard-shell pages** (`/summary` + 15 `/dashboard/*` sub-pages): reused `PageHeader`'s existing optional `id` prop (added in Addendum Z) — no component change needed, just setting the prop. Every `/dashboard/*` page also gets a second step pointing at the existing `dashboard-nav-sidebar` anchor ("jump to any section"), reusing the exact same anchor from Addendum Z. Old ids (`tour-priority-attention`, `tour-recommendations`, `tour-sprint`) renamed to a consistent `tour-header-*` scheme.
- **9 admin pages**: `AdminConsoleLayout` (`src/components/admin/AdminConsoleLayout.tsx`) gained a new optional `headerId?: string` prop, rendered as `id={headerId}` on the section wrapping its existing `<h1>{title}</h1>`/`<p>{description}</p>` — the tour copy for these pages reuses each page's own already-existing `title`/`description` props almost exactly (e.g. Audit Events, Import Logs, System Errors), so the tour never says anything the page header doesn't already say more slowly.
- **24 standalone pages** (Charts, Trends, Teams, Portfolio, Release Readiness, Flow Health, Sprint & Kanban, Delivery Mix, Explore, Customer, Roadmap, Forecast, Retro, Work Explorer, Data Quality, Snapshots, Column Mapping, Backend, Members, Landing, Glossary, Developer, Help, Profile): a plain `id="tour-header-*"` attribute added directly to each page's existing primary `<h1>` (or, for `/landing`, reusing an `id="landing-hero"` that already existed on its hero `<section>` — found while auditing, no edit needed). `/developer` and `/help` are documentation pages that render Markdown-to-HTML strings rather than plain JSX elements, so instead of a highlighted anchor they get a single centered step (`placement: 'center'`, no `targetId`) describing the page.
- Deliberately did **not** add id/tour coverage to `/dashboard` itself (an immediate `redirect()` to `/dashboard/priority-attention`, no content of its own).

**Docs updated to match the new model, not just left describing the old one:** `/help`'s "Product Tour" FAQ section (previously described the exact 8-step, 4-page walkthrough and a `resetTour()` console command that no longer exists) rewritten to describe the floating per-page button and the absence of any persisted state. `/developer`'s "Product tour" doc section rewritten from "(multi-page)" to "(per-page)", describing `PAGE_TOURS`, `PageTourButton`, and the `AdminConsoleLayout.headerId` addition.

**Testing:** `productTour.test.ts` fully rewritten — TC-PT-01 through TC-PT-07 (old `TOUR_STEPS`/dismiss-state tests) replaced with TC-PT-01 through TC-PT-06 for the new model: PAGE_TOURS has ≥40 routes with well-formed steps (TC-PT-01); every route key is a real `app/<route>/page.tsx` (TC-PT-02, catches a route added to the registry that doesn't exist, or a page renamed without updating the registry); every `targetId` is rendered somewhere in the source tree, reusing the same `idExistsInSource` grep helper from Addendum Z (TC-PT-03); `getPageTour` returns the right array for a known route and `null` for an unknown one (TC-PT-04); every step without a `targetId` uses `center` placement (TC-PT-05); every set `placement` value is one ProductTour actually supports (TC-PT-06).

**Verification:** `npx tsc --noEmit` clean. `npx eslint .` — 0 errors, 1,385 pre-existing warnings (down from the 1,524 documented in §60.1 on 2026-06-27, due to unrelated work merged since) — confirmed line-by-line that every file this change touched has zero *new* warnings versus before (the `id`/`headerId` attributes it adds are not style props, so they can't trip `react/forbid-dom-props`). `npx stylelint` clean on the new/changed SCSS modules. Full Jest suite: 109 suites / 1,014 tests passing (down from 1,018 in Addendum Z — expected, since 10 old `TOUR_STEPS`/dismiss-state tests were removed and 6 new per-page-model tests added; no test was deleted without a replacement covering the same real risk it caught). `npx next build` clean, all ~120 routes compiled including every route added to `PAGE_TOURS`. One real regression caught and fixed during verification: `productTour.test.ts`'s `idExistsInSource` helper originally only matched a literal `id="..."` in source, but the 9 admin pages set their anchor via `AdminConsoleLayout`'s new `headerId` prop (rendered internally as `id={headerId}`) rather than a literal `id=` at the call site — TC-PT-03 correctly failed on all 9 until the helper's grep pattern was extended to also match `headerId="..."`. **Not independently verified**: the actual rendered experience of all 49 page tours in a browser — confirming the floating button's position never collides with other floating UI on every page, confirming each highlight ring lands correctly, confirming popover text doesn't overflow its fixed 300px width on any of the longer descriptions — no browser-automation tool was available this session. Anchor existence and route validity are proven by the tests above; visual polish across 49 distinct pages was not visually confirmed.

## Addendum AB — Product Tour Deepened: Real Content Anchors on Every Page, Not Just the Header (2026-07-10)

**Same feature, one day later, direct user follow-up.** User feedback (verbatim): "the toure should cover every majer thing in the page, and provide what see in the page, not just go to two items that dont give any clue what page about." Addendum AA shipped 49 routes but most had only 1 step (the page header) or 2 (header + dashboard sidebar-nav) — real, but shallow: it named the page, not what was on it. This addendum adds 1-2 more steps to nearly every route, each pointing at an actual distinct content section (a chart, a table, a KPI strip, a filter bar) rather than the title.

**Also fixed in this addendum, found and corrected before it could ship broken:** `app/layout.tsx` imported `PageTourButton` in Addendum AA's own commit but never rendered it in JSX — the button never appeared anywhere. Caught from the user reporting "i cant find the tour icon in anywhere," fixed same-day in a direct commit to `main` (`db0c6b4`) before this addendum's work began.

**Content-gathering approach:** hand-authoring accurate anchor placement and copy for ~90 new content sections across 49 files required actually reading each page's full render logic (not just its header, as Addendum AA's lighter pass did) — parallelized across 6 background agents, each scoped to a distinct, non-overlapping file set (2 dashboard-page batches, 1 admin-page batch, 3 standalone-page batches), with explicit instructions to: read the entire file before picking anchors; anchor to a compact element (a heading, toolbar, or stat-strip row, not a full-height container — learned in Addendum AA that a full-height target pushes the popover off-screen); pick two sections genuinely distinct from each other and from the existing header step; and report back a description grounded in the real code (variable names, what's actually being mapped/rendered) rather than a guess. Every returned description was used as-is or near-verbatim, the same anti-fabrication discipline as Addendum AA.

**Registry restructured, not just extended:** each `/dashboard/*` route's step order changed from `[header, sidebar-nav]` to `[header, content-1, content-2, sidebar-nav]` — the navigational "jump elsewhere" step now comes last, after the page has actually explained itself, which reads better than leading with navigation. New steps default to `placement: 'bottom'` for the first content step and `placement: 'top'` for the second, since a second anchor is typically further down the page than the popover's fixed width can safely render below without approaching the viewport edge.

**`/developer` and `/help` gained real anchors too**, which Addendum AA explicitly could not do (their content renders as a Markdown-to-HTML string, with no plain JSX elements to attach an `id` to). Investigation this time found both pages have real structural JSX *outside* that markdown blob — a search box and section-nav sidebar on `/developer`, a search box and topic filter pills on `/help` — so both went from 1 centered step to 3 (1 centered welcome step + 2 real anchored steps), without touching the markdown-rendering path.

**One dynamic-anchor pattern introduced:** two pages (`/dashboard/actions`, `/dashboard/data-quality`) anchor to the *first* item in a repeated list (`id={i === 0 ? 'tour-section-actions-2' : undefined}`) rather than the whole list, to keep the anchor compact per the sizing rule above. This is a JS expression, not a literal `id="..."` string, so `productTour.test.ts`'s `idExistsInSource` grep helper needed a third pattern added (`id={[^}]*['"]<id>['"][^}]*}`) alongside the existing literal-`id=` and `headerId=` patterns — confirmed this addition is real and not just cosmetic by breaking one of the two dynamic ids, watching TC-PT-03 fail, then restoring it and watching it pass again.

**Scale:** 49 routes now carry 146 total steps (up from 49 in Addendum AA — essentially one step per route before, now averaging 3 per route), all 145 non-centered steps' `targetId`s verified to exist in source by TC-PT-03.

**Verification:** `npx tsc --noEmit` clean. `npx eslint .` — 0 errors, 1,385 warnings, identical to the pre-change count (confirmed by diffing per-file warning counts before/after: not one file gained a new violation, since every edit was either a plain `id` attribute or a `TourStep` object literal, neither of which can trip `react/forbid-dom-props`). `npx stylelint` — 2 pre-existing errors on `app/privacy` and `app/terms` (`-webkit-backdrop-filter` vendor-prefix), both untouched by this change, unrelated tech debt. Full Jest suite: 109 suites / 1,014 tests passing (same count as Addendum AA — no new test files added, `productTour.test.ts`'s existing TC-PT-01–06 cover the expanded registry structurally without needing new cases). `npx next build` clean. **Not independently verified**: the actual rendered experience in a browser — no browser-automation tool was available this session. In particular, the `top`/`bottom` placement heuristic for second-content-step anchors was chosen by reasoning about typical page layout, not by measuring real rendered positions on all 49 pages; a few pages with unusually short or long content between the two anchors may position the popover less ideally than intended.

## Addendum AC — Product Tour Popover Positioning Fix: No Longer Renders Off-Screen (2026-07-10)

**Confirmed the exact risk flagged (and left unverified) in Addendum AB.** User sent a screenshot of `/dashboard/flow-health` with the tour backdrop dimming the page but no popover visible anywhere on screen — a real, reproducible instance of the "popover positions off-screen" risk called out at the end of AB, not a hypothetical.

**Root cause:** `ProductTour.tsx`'s "above" placement computed `bottom: window.innerHeight - targetRect.top + GAP` with no clamping. On `/dashboard/flow-health`, the `items-table` step (added in Addendum AB) uses `placement: 'top'` and anchors near the top of the page content (just below the filter toolbar) — so `targetRect.top` is small, making `window.innerHeight - targetRect.top` large, which pushes the popover's `bottom` value far enough that its `top` edge lands above `y = 0`, off-screen. The "below" placement had the same unclamped-math class of bug in the other direction (a target near the bottom of a tall page could push the popover below the viewport), just not yet hit by a real anchor.

**Fix:** replaced the `top`/`bottom`-mixed, unclamped positioning with a single measure-then-clamp pass. `TourPopover` now holds a ref to its own root element, measures its real rendered height via `useLayoutEffect` (runs before paint, so there's no visible flash), computes a desired `top` for either placement (`target.bottom + GAP` for below, `target.top - GAP - measuredHeight` for above), then clamps that value to `[12, window.innerHeight - measuredHeight - 12]` — guaranteeing the popover's top and bottom edges both stay inside the viewport regardless of where the target sits or how long the description text makes the box. The horizontal clamp (`clampedLeft`) already existed and needed no change. `.popoverBelow`/`.popoverAbove` (two near-duplicate SCSS classes, one `top`-based and one `bottom`-based) collapsed into one `.popoverPositioned` class, since both placements now compute the same `top` CSS property.

**Verification:** `npx tsc --noEmit` clean. `npx eslint src/components/tour/ProductTour.tsx` — same 2 documented CSS-custom-property exceptions as before, 0 new warnings. `npx stylelint` clean on the changed module. Full Jest suite: 109 suites / 1,014 tests passing (unchanged — this is a positioning-math fix with no new testable surface `productTour.test.ts` covers; its existing tests validate step/anchor data, not runtime pixel positions). `npx next build` clean. **Not independently verified**: the fix was reasoned through analytically (traced the exact math that produced the reported off-screen result, confirmed the new clamp formula bounds both edges) rather than confirmed by rendering the page in a browser — no browser-automation tool was available this session. The specific `/dashboard/flow-health` case from the screenshot should be spot-checked once a browser is available, along with a page where the anchor sits near the *bottom* of a tall page (the "below" direction of the same class of bug, not yet directly observed).

## Addendum AD — Team Role View: Full Coaching Page Replacement (2026-07-12)

**Same-day full replacement, not an iteration.** Immediately after Addendum AC's sibling change (the "relevance-first" tab redesign, TODO-List.md `v4.21.0`) shipped, the user responded "No I dont like the style totaly" with a complete, detailed design brief for a "simple, light, role-based grid" — explicit product/UX/engineering requirements covering layout (3-column CSS Grid, no sidebar, no charts, no accordions), exact per-role rule/action/metric content, visual tone (soft status pills, light tinted headers, thin borders, minimal shadow), responsive breakpoints, a suggested TypeScript data contract (`RoleView`/`RoleRule`/`RoleAction`/`RoleMetric`/`Status`), and an explicit instruction to connect fields to real Jira data where possible and label fallbacks clearly otherwise.

**FR-411 (P1 — Done, 2026-07-12):** `/dashboard/coaching` MUST render a fixed, non-personalized 3-column grid — Scrum Master, Product Owner, Manager, in that order — identical for every viewer regardless of `AppRole`. The page MUST fetch only `DashboardMetrics` (via the existing `loadMetricsWithSource()`); it MUST NOT fetch the viewer's role or admin signals, since content no longer varies by viewer. `buildRoleGridView(metrics)` (`src/services/coaching/roleGridView.mapper.ts`) MUST build each `RoleView` directly from `DashboardMetrics` fields — not through the retired `generateAllCoachingInsights()`/`visibleCategoriesForRole()` orchestrator. Each `RoleView` MUST render as a `RoleColumn` containing exactly 3 sections in order — Rules to Monitor, Current Actions, Key Measures — via `RoleSection`/`RuleItem`/`ActionItem`/`MetricItem`/`StatusBadge` (all new, `src/components/dashboard/`). Every `RoleRule.status` MUST be one of `'critical' | 'risk' | 'review' | 'healthy'`, computed from a real `DashboardMetrics` field/threshold where a corresponding signal exists in the app, or fixed to `'healthy'` with an inline `// FALLBACK` comment where no such signal exists (never a fabricated non-`healthy` status). The grid MUST use `grid-template-columns: repeat(3, minmax(0, 1fr))` at desktop width, collapsing to 2 columns at ≤1099px and 1 column (stacked, Scrum Master → Product Owner → Manager, natural DOM order) at ≤699px.

**Reused thresholds, not invented ones:** the >35% capacity-load-share threshold (Manager's "Capacity overload" rule) and the <60% delivery-confidence threshold (Manager's "Delivery forecasts" rule) are the exact literal values already hardcoded in `src/services/coaching/generators/scrumMaster.generator.ts`, `engineeringManager.generator.ts`, and `deliveryManager.generator.ts` — copied, not derived independently, so the same real-world condition means the same thing in both the old (now-orphaned) generators and the new mapper. The one genuinely new threshold — 20% carryover rate for Scrum Master's "Carry-over" rule — has no prior app-wide equivalent; it's a named constant (`CARRYOVER_AT_RISK_THRESHOLD_PCT`) with a comment flagging it as a reasonable default pending a real team-configured value, per CLAUDE.md §7's guidance that this class of value is a good future Level-3 runtime-configuration candidate.

**Real data vs. documented derivations vs. fallback — the user explicitly asked this be stated, not left implicit:**
- **Direct existing fields (real):** Blocked items, Overloaded members, Critical epics, Overdue items, Orphan items, Delivery health score, Critical items (`metrics.flow.critical`), Cycle time.
- **Simple documented derivations of existing fields (real, not new calculations):** Carry-over % (`totalCarryoverCount / totalCommittedCount` across recorded sprints), Sprint goal coverage % (share of recorded sprints whose `goalOutcome` was `'Met'`), Forecast variance % (`100 − overallDeliveryConfidence`, an inverse framing of an existing score).
- **Explicitly labeled FALLBACK (no real data exists anywhere in the app):** Scrum Master's "Retro actions completed" metric, and the `'healthy'` default status on exactly 2 of 12 rules (retrospective-action ownership; priority-change visibility) — both because no ownership/audit-trail tracking exists for these yet, called out in code comments and in `/help`.
- **Deliberate label corrections from the user's own example content:** the design brief's example labels "Overloaded teams" and "Critical risks" were relabeled "Overloaded members" and "Critical items" — the backing data (`CapacityEntry.assignee`, `FlowSummary.critical`) is per-person and per-issue respectively, not per-team or a risk-register concept, and CLAUDE.md §29 requires domain language to match what the data actually represents.

**Orphaned subsystem — removed 2026-07-14:** the entire prior Role-Based Coaching generator/orchestrator/confidence/trend/evidence-link subsystem (7 generators, orchestrator, `ceremonyAdvice.service.ts`, `coachingConfidence.service.ts`, `coachingTrend.service.ts`, `coachingEvidenceLink.ts`, `coachingBadge.ts`, `adminSignals.service.ts`, `app/api/coaching/admin-signals/route.ts`, `src/types/roleBasedCoaching.ts`) had zero callers from any page or component — confirmed by repo-wide grep excluding each file's own tests. This was real, tested, still-plausibly-reusable domain logic, not accidental dead code from a rename; it was initially left in place and tracked as `ORPHAN-03` in `TODO-List.md` Section 18f for an explicit future decision rather than deleted alongside the unrelated redesign that surfaced it, per the CLAUDE.md §5 "no unowned code left undecided" principle already applied to `ORPHAN-01`/`ORPHAN-02`. That decision was made during the full-application product audit (`docs/product-audit/04-remove-merge-keep.md` R-13): not on the roadmap for reactivation, so the entire subsystem was removed, along with the separately-tracked `orphanRelation.service.ts`'s dead `detectOrphans()` (R-14, bundled into the same disposition per that finding's own instruction) — 20 files, 1,306 lines total. `CoachingInsightCard.tsx`/`CoachingOtherRoles.tsx` — pure presentational components with no domain logic and, after this change, zero remaining callers — were deleted outright rather than flagged, consistent with how `CoachingCategoryTabs.tsx` was handled in Addendum AC's sibling change hours earlier.

**Self-review pass caught 3 real issues before merge, all fixed:** (1) the Manager "Capacity overload" rule and its paired action omitted the `metrics.capacity.length > 2` small-team guard that `scrumMaster.generator.ts`/`engineeringManager.generator.ts` apply before treating an overloaded member as a real signal — a 1-2 person team trivially has someone over 35% load share; added the same guard (`capacitySignalMeaningful`). (2) The Manager "Rebalance N overloaded team member(s)" action rendered as "Rebalance any overloaded team members" when there was nothing to rebalance, contradicting the paired rule's `healthy` status — reworded to a distinct, positive-toned action ("Confirm team capacity stays balanced") when `overloaded.length` is 0, and gated on the same small-team guard as the rule so the two never disagree. (3) The header/tour copy read as if the page were still personalized per viewer ("each role sees only...") — the page header keeps the user's own literal requested copy (accurate read as "each column shows only its own role's content"), but the tour step's wording (not user-specified) was clarified to state explicitly that every visitor sees the same grid.

**Testing:** new `src/__tests__/roleGridView.test.ts` (`TC-RGV-01`–`08`, 8 assertions) covers: the fixed 3-role output order; an all-healthy baseline on clean data; the blocked-items→critical status path; the carryover-rate derivation and its 20% threshold (a real off-by-fixture bug was caught and fixed during authoring — an early version of this test left `totalCommitted` stale at 0 from an unrelated default while overriding `sprints`, which the mapper's own zero-guard correctly treated as "no data" rather than dividing by zero; the fixture, not the mapper, was wrong); the zero-sprints/no-NaN case for sprint goal coverage; the Manager forecast-variance derivation and its <60% threshold; the Manager capacity-overload threshold in a team larger than 2; and (added during the self-review fix above) the small-team guard suppressing both the rule status and the action text together for a team of ≤2. `roleBasedCoaching.test.ts`, `coachingTrend.test.ts`, `coachingEvidenceLink.test.ts` continue to pass unmodified (they test the orphaned subsystem directly, not through any page).

**Verification:** `npx tsc --noEmit` clean. `npx eslint` — 0 errors, 0 new warnings on every new/touched file; the CLAUDE.md §60 project-wide inline-style count actually *dropped* to 1,274/86 files (from 1,276/87) since deleting `CoachingInsightCard.tsx` removed its 2 documented CSS-custom-property-exception warnings along with it. `npx stylelint` clean on all 7 new SCSS modules — all values are design tokens, no hardcoded colors; status color is applied via `data-status` attribute selectors in SCSS, never computed as a raw color value in TypeScript (CLAUDE.md §28). Full Jest suite: 110 suites / 1,022 tests passing (one unrelated pre-existing Jest-worker `SIGSEGV` flake on `throughput.test.ts` under default parallel execution, the same class of flake documented for `forecastEngine.test.ts` in earlier addenda — confirmed passing cleanly in isolation with `--runInBand`). `npx next build` clean, `/dashboard/coaching` compiles to a smaller bundle (6.53 kB) than either prior version. **Not independently verified**: the actual rendered appearance and responsive behavior in a browser at desktop/tablet/mobile widths — no browser-automation tool was available this session. Verified analytically instead: every color/spacing/radius value traced back to a token in `_tokens.scss`, and the grid's breakpoint math (`repeat(3,…)` → `repeat(2,…)` at ≤1099px → `1fr` at ≤699px) re-read directly against the spec's own three-tier desktop/tablet/mobile requirement. A manual visual pass is recommended before this is considered fully done.

---

**FR-412 (P0 — Done, 2026-07-31):** `P0B-01` — registration's "primary role" persona list, and the concept of secondary roles, MUST match the soft-launch master plan (§4.1): "one primary role and zero or more secondary roles," suggested list of 11 (Scrum Master, Agile Coach, Product Owner, Project Manager, Delivery Manager, Engineering Manager, Team Lead, Executive, Jira Administrator, Consultant, Other). `src/lib/personas.ts`'s `PERSONAS` restored from 5 to all 11 (reversing the FR-395-era 4.26.1 trim, per explicit user instruction — a deliberate reversal, not a silent one). `POST /api/auth/register` accepts an optional `secondaryPersonas: string[]`, validated by dedup + `PERSONAS` membership + excluding whatever was chosen as primary; invalid/missing values are silently dropped rather than a 400, since the field is optional, checkbox-driven in the real UI, and analytics-only — no permission surface exists for it to protect (confirmed: `role: 'user'` is hardcoded server-side at registration regardless of submitted persona, and neither `persona` nor `secondaryPersonas` is read in any authorization check anywhere in `app/api`). New `User.secondaryPersonas String[] @default([])` column, additive migration `20260731000001_add_secondary_personas`. `/register` gained a `<fieldset>`/`<legend>` checkbox group (the correct native semantic element for a checkbox group per CLAUDE.md §26.1 — no existing multi-select component in this codebase to reuse instead) listing the other 10 roles, dynamically excluding whichever is currently selected as primary.

**Persona list expansion forced a structural completeness fix, not an optional one:** `src/config/personaFocus.config.ts`'s `PERSONA_FOCUS: Record<Persona, PersonaFocusConfig>` is a TypeScript mapped type keyed by the `Persona` union — expanding `PERSONAS` to 11 values fails `tsc --noEmit` immediately unless all 11 keys exist. Authored the 6 missing entries (Agile Coach, Delivery Manager, Team Lead, Jira Administrator, Consultant, Other), reusing only routes already referenced by the 5 pre-existing entries (`/dashboard/trends`, `/dashboard/ownership`, `/dashboard/priority-attention`, `/dashboard/flow-health`, `/dashboard/data-quality`, `/dashboard/epic-readiness`, `/dashboard/key-metrics`, `/dashboard/labels`, `/dashboard/summary`) — no new routes invented. The existing completeness test (`src/__tests__/personaPreview.test.ts`) needed no code change — it already iterates `PERSONAS` generically, so it simply started requiring 11 entries instead of 5.

**Tracker status corrected, not just the persona gap:** `TODO-List.md`'s `P0B-01` row read "❌ Not started," but direct code audit found registration, email verification, and workspace/entitlement creation were already fully implemented and live (EP-011/EP-012) — this ticket's real remaining scope was narrower than its own description implied. `product/DEVELOPER_GUIDE.md` separately and independently claimed (in 4 places — lines ~78, 152, 172, 479) that public registration is disabled and `POST /api/auth/register` returns 403 "by product policy" — directly contradicted by the live route and by `product/MANUAL_TESTS.md` §14, which documents testing the real flow against production (`https://delivery-clarity.onrender.com/register`). All 4 corrected.

**Design decisions, not defaults:** no join table or JSON column for `secondaryPersonas` — nothing filters/queries on persona anywhere today (confirmed via grep), so a join table would be speculative architecture (CLAUDE.md §5.5); Postgres `text[]` doesn't foreclose a future filter need either, since Postgres array operators (`has`/`hasSome`/`hasEvery`) exist if ever required. No secondary-role cap/exclusion-combination rule was invented — the master plan says only "zero or more," and the UI's primary-exclusion already self-limits to 10.

**Risk:** classified Medium, not High, despite the schema change (CLAUDE.md §53) — additive-only (`ADD COLUMN ... DEFAULT`), non-destructive, and zero access-control blast radius (confirmed above). Migration still verified via a real `prisma migrate deploy` against a live Postgres instance (not just local `db push`) before merge, matching the verification bar every other additive column in this file's history has been held to.

**Testing:** `src/__tests__/register.test.ts` — `TC-REG-03` (valid `secondaryPersonas` persists deduped/filtered), `TC-REG-04` (invalid value silently dropped, not a 400), `TC-REG-05` (primary value duplicated in `secondaryPersonas` is excluded), `TC-REG-06` (missing field defaults to `[]`), alongside the pre-existing `TC-REG-01`/`02`.

**Verification:** `npx tsc --noEmit` clean (including the forced `personaFocus.config.ts` completeness). `npx eslint` clean, no new warnings. Full Jest suite passing, including the persona-focus completeness test now covering 11 entries and the 4 new `TC-REG` cases. `npx prisma migrate deploy` applied cleanly against a live Postgres (Neon) instance. `npm run build` clean. **Not independently verified:** the actual rendered appearance of the new checkbox group in a live browser at desktop/mobile widths — no browser-automation tool was available this session; verified analytically instead via the reused `.consentLabel`/`.consentCheck`/`.consentText` SCSS classes (already proven in the existing consent checkbox on the same page) and by confirming the primary-role exclusion logic directly in the component code.

---

**FR-413 (P0 — Done, 2026-08-01):** `P0B-02` — trial entitlement MUST implement master plan §4.1 in full: one successful free analysis, 30 days of workspace access, failed validation/processing attempts MUST NOT consume entitlement, one optional replacement upload within 24h of the first successful analysis, and after expiry the private workspace MUST be disabled while a demo dataset remains available. The core state machine (`src/lib/entitlement.ts`, EP-015) already satisfied everything except the last two. `checkUploadEntitlement` now detects an in-window, not-yet-used replacement (`!ent.replacementUsedAt && ent.consumedAt && (Date.now() - ent.consumedAt.getTime()) <= 24h`) and returns `{allowed: true, isReplacement: true}`; `beginReplacementUpload`/`consumeReplacementUpload`/`revertReplacementUpload` mirror the existing begin/consume/revert trio but lock via `replacementUsedAt` instead of `status`, so `status` MUST remain `'consumed'` throughout — a failed replacement reverting `status` to `'eligible'` would grant an unearned fresh 30-day trial. New additive `Entitlement.replacementUsedAt DateTime?` column, migration `20260801000001_add_entitlement_replacement`.

**Real bypass closed, not merely a missing feature:** `app/api/upload/merge/route.ts` (confirmed via full read before this change) imported nothing from `@/lib/entitlement` — a blocked non-admin user could fully bypass the trial gate through the multi-file merge flow. Given the identical check/lock/consume/revert wiring as `app/api/upload/route.ts`, since the master plan doesn't distinguish "one successful free analysis" by upload mechanism. Two more gaps surfaced in the same file while fixing this, both closed in the same patch: no `ImportLog` row was ever created (now required as `consumeEntitlement`'s FK target — wrapped in `prisma.$transaction`, mirroring the single-file route exactly), and the EP-011 email-verification gate present on the single-file route was missing here.

**Expiry enforcement — the harder design problem, since no middleware exists in this repo:** before this change, `checkUploadEntitlement` was the *only* place entitlement was ever checked anywhere in the app — a `consumed`/`expired`/`suspended` user's dashboard rendered and exported normally forever; only starting a *new* upload was blocked. Rather than add per-page checks across the ~17 pages that call `loadMetricsWithSource()`, or introduce `middleware.ts` (new architecture this repo has deliberately not adopted), gated the one true chokepoint: `GET /api/metrics/latest`, which every one of those pages' calls to `loadMetricsWithSource()` ultimately hits for a cloud-mode (non-local-storage-mode) user. For `session.role !== 'admin'`, a `status === 'expired'` entitlement now short-circuits to `{available: false, reason: 'expired', message: '...'}` before any scope-key resolution or file read runs.

**A verified, not assumed, correction to the initial expiry design:** `src/lib/storage.ts`'s `loadMetricsWithSource()` was read in full before finalizing this design, and its existing local-fallback branch (falls back to a `localStorage`-cached copy, tagged with `fallbackUsed: true`, whenever one exists and is verified as belonging to the current account) would have silently defeated the new expiry check for almost every real expired user — consuming a trial requires having loaded the dashboard at least once, so a stale cache is the norm, not the exception, for exactly this population. Fixed by special-casing `data.reason === 'expired'` to skip the local-fallback branch entirely and return `{source: 'none', metrics: null}` unconditionally.

**"Demo dataset" prerequisite, found and fixed mid-pass:** the master plan's post-expiry design ("preserve access to a demo dataset while disabling the expired private workspace") only makes sense if the existing "Try a sample first" feature (`app/page.tsx`'s `handleSampleData()`) is itself free and non-destructive. It wasn't — it called the same `handleFile()` → `POST /api/upload` path as a real upload, meaning trying the sample **consumed the real trial entitlement** and overwrote the caller's real server-scoped dashboard data. Rewired to always process the sample through `processFileLocally()` (a pure client-side parse→validate→calculate pipeline, already used elsewhere in this file for local-storage-mode uploads), regardless of `dataStorageMode`, so it can never touch entitlement or the server. Known, explicitly accepted residual: a cloud-mode user could still see their real (or absent) server data swap back in on a later remount, since `loadMetricsWithSource()` prefers the server for cloud-mode accounts — a dedicated non-network "demo mode" flag would close this fully; not built in this pass.

**Redirect-message delivery — a second verified-not-assumed fix:** the initial design assumed every page redirecting on null metrics used `redirectWithLoadError(router, message)` (which stores an explanatory `sessionStorage` signal `/` reads on mount). Reading `app/dashboard/layout.tsx` and `app/forecast/page.tsx` directly showed this is false — both do a bare `router.replace('/')` on null data with no message at all. Rather than touch every one of the ~17 affected pages, extracted `redirectWithLoadError`'s signal-storage half into a standalone `setLoadErrorSignal()` (`src/lib/loadErrorSignal.ts`) and call it directly from `loadMetricsWithSource()`'s new expired branch — the message now reaches `/` regardless of which redirect pattern the calling page happens to use.

**Deliberately not built, each with a traced reason:**
- Mutating `Workspace.status` to `'suspended'` on expiry, despite the master plan's literal "disabling the expired private workspace" wording — traced its only two consumers (`getWorkspaceForUser()`'s `status: 'active'` filter; `getMetricsScopeKeyForUser()`'s fallback to a *different* scope key, `user:${userId}`, when no active workspace resolves) and found flipping it would silently redirect metrics reads/writes to a different storage key than real data was written under, making the dashboard look freshly empty rather than "disabled with demo fallback" — worse than doing nothing. Entitlement-status-alone carries no such landmine.
- Admin suspend/restore UI for entitlement — `restoreEntitlement()`/the `suspended` status already exist with zero call sites; wiring an admin UI to them is `P0B-13`'s scope ("Minimum admin operations"), not this ticket's.
- Gating `/api/snapshots`, `/api/trends`, `/api/imports/export`, etc. individually — a determined expired user hitting those directly bypasses the one chokepoint this pass closed; small, explicitly accepted residual gap, not solved here.
- `src/lib/localUpload.ts`'s local-storage-mode processing touching entitlement at all — deliberate, pre-existing privacy design (FR-377: local mode never sends file contents to the server, so there is nothing to gate), not a bypass.

**Risk:** classified High per CLAUDE.md §53 — this touches authorization (entitlement is access control), a database migration, the app's critical upload workflow, and closes a live bypass, unlike `P0B-01`'s zero-blast-radius persona work. Full CI, migration validation against a live instance, and a `/security-review` pass were run before merge, with particular attention to the replacement-window race condition (`beginReplacementUpload`'s `updateMany` count-based optimistic lock) and confirming no code path flips `status` away from `'consumed'` on a failed replacement.

**Testing:** see the 4.35.0 revision-history entry above for the full test-file list; the single most important regression test in this ticket is `uploadUserId.test.ts`'s assertion that a *failed* replacement upload reverts via `replacementUsedAt` and leaves `status` at `'consumed'`, never `'eligible'` — getting this wrong grants free extra trials.

**Verification:** `npx tsc --noEmit` clean. `npx eslint` clean, no new warnings. Full Jest suite passing. `npx prisma migrate deploy` applied cleanly against a live Postgres (Neon) instance. `npm run build` clean. **Not independently verified, disclosed rather than assumed:** `loadMetricsWithSource()`'s and `handleSampleData()`'s actual runtime behavior in a live browser — this repo has no jsdom or component-testing infrastructure anywhere (confirmed via `product/DEVELOPER_GUIDE.md`'s Tech Stack table and the identical, previously-noted gap at FR-396), so these were verified via direct code reading and are recommended for manual QA before this is considered fully closed: upload → consume → replacement within 24h → third attempt blocked; try the sample and confirm no entitlement/server-data change; simulate an expired entitlement with a pre-existing `localStorage` cache present and confirm the real dashboard is not shown from that cache.

---

**FR-414 (P0 — Done, 2026-08-01):** `P0B-03` — consent MUST be a real, retained record per purpose, not a flat overwrite-once field, and the app MUST offer a self-service "analytics choices" control per the soft-launch master plan (§6.3 data model, `user_consents`; NFRs, "Privacy: ... consent records ... and no unneeded personal analytics"). Before this change, consent was exactly two `User` columns (`termsAcceptedAt`/`termsVersion`, EP-014), written once at registration and never read or written anywhere else in the codebase (confirmed via repo-wide grep) — despite `src/lib/legal-i18n/en.ts` §6 already promising "Consent records: lifetime of account plus 6 years for legal compliance purposes." There was also no "analytics choices" concept at all, because no analytics or event-tracking system exists anywhere in this repo yet (confirmed via grep for `gtag`/`GA4`/`trackEvent`/`product_events` — none found; `P0B-05`–`07`, the event taxonomy/queue/ingestion tickets, are separate and not yet built). `/terms`, `/privacy` (7-language `legal-i18n`), and the required registration consent checkbox (EP-014) were already fully built and are unchanged by this ticket — their substantive legal content was deliberately not edited (a 7-language legal-content change needs real translation review, out of scope for an engineering ticket, and nothing added here contradicts anything already promised).

New `Consent` model (`prisma/schema.prisma`, additive migration `20260801000002_add_consent`): `id`, `userId`, `purpose` (`"terms_and_privacy"` | `"analytics"`), `granted`, `version`, `source` (`"registration"` | `"settings"`), `ipAddress`, `userAgent`, `createdAt` — **append-only**, never updated in place; one row per grant/withdrawal event. Current status for a purpose is always the latest row for `(userId, purpose)`. New `src/lib/consent.ts`: `recordConsent(userId, purpose, granted, opts, tx?)` inserts a row (accepts an optional transaction client, mirroring `src/lib/entitlement.ts`'s `consumeEntitlement` pattern); `getConsentStatus(userId)` reads the latest row per purpose and, for `terms_and_privacy` only, **falls back to the legacy `User.termsAcceptedAt`/`termsVersion` fields when no `Consent` row exists** — true for every account that registered before this shipped, so no existing user's status silently reports as "not accepted."

`POST /api/auth/register` now calls `recordConsent(user.id, 'terms_and_privacy', true, {version: CURRENT_TERMS_VERSION, source: 'registration', ipAddress}, tx)` inside the same `prisma.$transaction` as user creation — the existing `termsAcceptedAt`/`termsVersion` writes on `User` are unchanged, kept as a fast-path duplicate rather than migrated away from, so the one pre-existing call site carries zero behavior risk.

New `GET`/`POST /api/consent` (session-authenticated, mirroring `app/api/profile/route.ts`'s auth pattern): `GET` returns `getConsentStatus()`. `POST` accepts only `{purpose: 'analytics', granted: boolean}` — a request for `purpose: 'terms_and_privacy'` is rejected with 400, since withdrawing *required* consent while keeping the account active isn't a state this ticket models; the privacy policy already frames full withdrawal as equivalent to requesting account deletion, correctly deferred to `P0B-04` ("Data lifecycle"). A successful `POST` also writes an `AuditEvent` (`eventType: 'consent_update'`, mirroring `profile_storage_mode_change`'s existing pattern).

New Settings → **Privacy** tab (`src/components/settings/PrivacyTab.tsx`, added to `app/profile/page.tsx`'s `SETTINGS_TABS`, following the established `StorageTab.tsx` pattern exactly): shows the user's terms-acceptance record (version + date, falling back gracefully when no version data exists) with links to `/terms`/`/privacy` and an explanation that withdrawing required consent means emailing to request account deletion; and an **Analytics** toggle, defaulting to off, whose own copy is explicit that "We don't currently collect product analytics — this preference will be honored once we do" — deliberately not implying live behavior that doesn't exist yet. A second mandatory checkbox at registration was considered and rejected: analytics consent has no functional effect today, so adding checkbox friction to signup for a purpose nothing yet gates would be pure UX cost with no present benefit; an opt-in Settings toggle is the correct home. Consequently, `analytics` rows are only ever created when a user actually visits Settings and makes a choice — `getConsentStatus()`'s `analytics.decided` flag distinguishes "never asked" (`false`) from "explicitly declined" (`false` + `decided: true`), so no fabricated decision is ever recorded for a choice a user never made.

**Deliberately not built, each with a traced reason:**
- Account deletion, data export, and any automated retention-purge job for any data category — `P0B-04` ("Data lifecycle"), a separate tracker item that explicitly depends on this one.
- Forcing re-consent when `termsVersion` changes (blocking login/upload until re-accepted) — not promised anywhere today; a reasonable, explicitly-deferred follow-up, not silently dropped.
- Any GA4/cookie-consent-banner UI — no third-party analytics exists yet in this app; `P1-06` (GA4 integration) is the ticket that will actually wire it, and it explicitly depends on `P0B-03`/`P0B-05` — confirming this ticket correctly built exactly the prerequisite consent record that later ticket needs, not premature scope.
- Editing the 7-language `legal-i18n` policy text (see above).

**Risk:** classified Medium per CLAUDE.md §53 — new table + additive migration, but unlike `P0B-02`'s entitlement work, nothing here gates authorization or closes a live bypass: the analytics toggle has zero functional consequence until `P0B-05`+ ships, and required terms-consent at registration is unchanged. Still held to the same live-DB migration verification bar as every prior High/Medium item.

**Testing:** new `src/__tests__/consent.test.ts` — `recordConsent` writes the expected fields via both the default client and a passed transaction client; `getConsentStatus`'s latest-row-wins behavior across multiple rows, its legacy-field fallback when no `Consent` row exists, and the `analytics.decided` true/false distinction. New `src/__tests__/consentApi.test.ts` — `GET`/`POST` auth (401 when logged out), `POST` granting/withdrawing analytics, `POST` rejecting `purpose: 'terms_and_privacy'` with 400, `POST` rejecting a non-boolean `granted`. Extended `src/__tests__/register.test.ts` (`TC-REG-07`) — asserts the `terms_and_privacy` consent row is recorded inside the same transaction as user creation, alongside the existing persona/secondary-persona coverage.

**Verification:** `npx tsc --noEmit` clean (including regenerating the Prisma client for the new `Consent` model). `npx eslint` clean, no new warnings (same pre-existing 8-warning baseline). Full Jest suite passing, 1141/1141 tests across 125 suites (one unrelated single-worker `SIGSEGV` crash on `navGroupsForRole.test.ts` during the full run, confirmed a flake by re-running that file alone in isolation — passed cleanly, 19/19, no relation to any file this ticket touched). `npm run validate` (`prisma validate` + `prisma generate` + typecheck + `lint:css`) clean. `npx prisma migrate deploy` applied `20260801000002_add_consent` cleanly against the live Postgres (Neon) instance. `npm run build` succeeds, `/api/consent` present in the route manifest. **Not independently verified, disclosed rather than assumed:** `PrivacyTab.tsx`'s actual rendered behavior in a live browser — this repo has no jsdom or component-testing infrastructure anywhere (Node-only Jest, the same confirmed gap noted at FR-396/FR-413) — verified via direct code reading instead, and recommended for manual QA before this is considered fully closed: register a new account → Settings → Privacy shows "Accepted version v1" with today's date; toggle Analytics on/off → reload → choice persists; an existing pre-migration account → Settings → Privacy still shows correct terms-acceptance info via the legacy-field fallback, not an error.

---

**FR-415 (P0 — Done, 2026-08-02):** `P0B-04` — the app MUST offer self-service account deletion and export, and MUST enforce the retention windows the Privacy Policy already publishes, per the soft-launch master plan (§6.3: "Create account deletion, consent withdrawal, export and expiry jobs before public launch"). Research before implementation found: no self-service deletion or export existed anywhere (`DELETE /api/admin/users` was admin-only, couldn't target its own account; `GET /api/imports/export` exported only `ImportLog` metadata as `.xlsx`); 12 of 14 `User`-FK'd Prisma models already `onDelete: Cascade`, so a hard `prisma.user.delete()` needs almost no special-casing (`AuditEvent.userId` is `SetNull` by design, matching its independent 12-month audit retention; `Feedback`/`AppError` have no FK at all, already orphan-safe); `GET /api/auth/me` (EP-010) already destroys the session and 401s on the next request for any `!isActive` or deleted user, with zero new code needed; and both upload routes (`app/api/upload/route.ts`, `app/api/upload/merge/route.ts`) were traced in full and confirmed to never persist raw uploaded file bytes anywhere — only the computed `DashboardMetrics` is written (a filesystem JSON file, not a DB row).

**Deletion is a soft-then-hard two-phase design, not an immediate delete** (explicit product decision, given the destructive/irreversible nature of the operation): `POST /api/account/delete` requires re-entering the current password (`verifyPassword`, same function and same 10-attempts/15-minute rate-limit bar as `POST /api/auth/change-password`), then sets `isActive: false` and a new `User.deletionRequestedAt DateTime?` (additive migration `20260802000001_add_deletion_requested_at`) in one update, destroys the iron-session, and writes a final `AuditEvent`. The account is locked out immediately — every other open tab/device is ejected on its next `GET /api/auth/me` call via the pre-existing EP-010 check, not any new gating logic. A new daily cron (`.github/workflows/data-retention.yml`, `workflow_dispatch` + `schedule`, same shape as the existing `db-backup.yml`) runs `scripts/purge-expired-data.mjs`, which hard-deletes any `User` row whose `deletionRequestedAt` is more than 7 days old — comfortably inside the Privacy Policy's already-published "deleted within 30 days of confirmed account deletion" SLA, while leaving a real, admin-actionable recovery window. `PATCH /api/admin/users` (reactivating a user, `isActive: true`) now also clears `deletionRequestedAt` in the same `prisma.user.update` call — without this the grace window would be uncancelable by design, defeating its own purpose.

**Export** (`GET /api/account/export`) returns a single downloadable JSON document — deliberately not CSV or a zip: GDPR Article 20 asks for "a structured, commonly used and machine-readable format," which JSON satisfies directly, and this sidesteps the CSV/spreadsheet formula-injection surface (CLAUDE.md §38.5) entirely rather than needing to defend against it. The bundle: profile fields (explicitly excluding `passwordHash` and all token fields — asserted by a dedicated regression test), full `Consent` history (every row, not just current status — this literally is the record Article 15/20 covers), entitlement status, workspace summary, `ImportLog` metadata, full `DashboardSnapshot` rows including `metricsJson`, and the user's current live dashboard metrics (`readLatestMetrics()` via the same `getMetricsScopeKeyForUser()` scope-key resolution the upload routes already use). Local-storage-mode accounts (EP-017) have nothing server-side to export by definition, not a gap. New `src/lib/accountLifecycle.ts` (`requestAccountDeletion`/`cancelAccountDeletion`/`exportAccountData`) backs both new API routes and is the unit-tested source of truth.

**"Uploaded-original policy" is a documentation-only deliverable, verified rather than assumed** — since no raw uploaded file bytes are ever stored server-side (see above), there is nothing to build a retention/purge job for; recorded here as a deliberate, checked finding rather than a silently skipped tracker line.

**Retention enforcement** closes exactly the two currently-zero-enforcement categories the Privacy Policy already promises a concrete window for: `AuditEvent` purged past 12 months, `AppError` purged past 90 days (keyed off `lastSeenAt`, not `firstSeenAt`, so an error still actively recurring is never purged only because it was first seen long ago) — both confirmed via grep to have had no pruning logic anywhere before this. The existing, previously admin-click-only `applyRetentionPolicy()` (`ImportLog`/`DashboardSnapshot`) is now also run automatically by the same daily job, reading the admin's saved settings from the DB-backed global `AppSetting` row directly (a safe no-op by default, since `DEFAULT_SETTINGS.retentionDays === -1`/both auto-delete flags default `false`). `Consent` (6-year retention horizon) and `LoginAttempt` (`"pruned after 1 hour"`, already correctly self-pruning inline on every login/register call, confirmed via code reading) were deliberately left untouched — no near-term gap to close for either, and CLAUDE.md §5.5 counsels against building unverifiable, unexercised purge logic for a multi-year horizon.

**Scheduled-script architecture, following existing precedent rather than inventing a new one:** this repo has no `ts-node`/`tsx` runtime wired for standalone scripts (confirmed via `prisma/seed.mjs`, which is plain JS instantiating its own `PrismaClient` directly rather than importing any `src/` TypeScript module) and Render's free-tier web service has no cron infrastructure of its own (the same constraint `P0A-06`'s `db-backup.yml` was built around). `scripts/purge-expired-data.mjs` follows `seed.mjs`'s exact shape — a standalone script with its own `new PrismaClient()` and its own inline query logic — rather than attempting to import `accountLifecycle.ts` from a plain `.mjs` file, which Node cannot do without a build step this repo doesn't have. This is accepted, bounded duplication (a handful of simple `deleteMany`/`delete` calls, not complex logic), consistent with how `seed.mjs` and the app's own user-creation code already duplicate similarly small amounts of logic rather than share it across the TS/plain-JS boundary. Reuses the `NEON_DIRECT_DATABASE_URL` secret already added for `db-backup.yml` — no new secret required.

**Found and fixed while directly touching this exact area (CLAUDE.md §61), not original scope:** `app/api/admin/cleanup/route.ts` (the admin "Run cleanup now" button) was reading retention settings via the legacy filesystem-only `readSettings()`, while the real settings UI (`POST /api/admin/settings`) saves through the DB-backed `readSettingsForUser`/`writeSettingsForUser` pair — two disconnected stores, meaning an admin's just-saved retention-days change could silently fail to apply the next time they clicked "Run cleanup now." Fixed with a one-line swap to `readSettingsForUser`, the same source this ticket's new cron script also reads from.

**Deliberately deferred, named, not silently skipped:** updating `src/lib/legal-i18n/en.ts`'s "email us to request deletion" instruction to also describe the new self-service option — per `P0B-03`'s already-established boundary, editing the substantive 7-language legal-policy content needs real translation review, out of scope for an engineering ticket; the existing text isn't contradicted by adding a self-service path (a superset, not a conflict), so this doesn't block the ticket.

**Testing:** new `src/__tests__/accountLifecycle.test.ts` (`requestAccountDeletion`/`cancelAccountDeletion` state changes; `exportAccountData`'s field-exclusion regression test and full/empty-state shape coverage). New `src/__tests__/accountDeleteApi.test.ts` (wrong password rejected without side effects; correct password locks out + destroys session + audits; rate-limit 429; unauthenticated 401). New `src/__tests__/accountExportApi.test.ts` (authenticated export shape; unauthenticated 401). Extended `src/__tests__/adminUsers.test.ts` (2 new cases: reactivating clears `deletionRequestedAt`; disabling never touches it).

**Verification:** `npx tsc --noEmit` clean. `npx eslint` clean, same pre-existing 8-warning baseline, no new warnings. Full Jest suite passing, 128 suites / 1174 tests. `npm run validate` clean. `npx prisma migrate deploy` applied `20260802000001_add_deletion_requested_at` cleanly against the live Postgres (Neon) instance. `npm run build` succeeds, `/api/account/delete` and `/api/account/export` present in the route manifest. **Not independently verified, disclosed rather than assumed:** the actual cron workflow's first live run (requires `NEON_DIRECT_DATABASE_URL` to already be configured as a repo secret, per `P0A-06` — not something this sandbox can trigger or observe), and `PrivacyTab.tsx`'s new Export/Delete UI in a live browser — same disclosed no-jsdom-infrastructure gap as every prior P0-B ticket. Recommended manual QA before considered fully closed: request deletion with the wrong password (rejected, account untouched) → correct password (immediately locked out, `/api/auth/me` now 401) → admin reactivates within the grace window (`deletionRequestedAt` cleared, login works again) → download an export and confirm it's valid JSON with no password hash present.

---

**FR-416 (P0 — Done, 2026-08-02):** `P0B-09` — the feedback popup MUST offer screenshot capture, per the soft-launch master plan's feedback requirements. Research before implementation found the tracker's "❌ Not started" status badly stale: `src/components/feedback/FeedbackButton.tsx` (accessible, focus-trapped, globally mounted modal), `POST /api/feedback` (rate-limited, validated, auto-captures page/browser/version/session identity, never Jira-derived data — confirmed via the route's own header comment and by reading it in full), the `Feedback` model's `impactLevel`/7-state `status` workflow, and a complete `app/admin/feedback/` triage UI were all already built and tagged `P0B-09` in their own source comments. A repo-wide grep for `html2canvas`/`getDisplayMedia`/any canvas-to-image utility confirmed the one genuine gap: screenshot capture, the sole remaining requirement this ticket closes.

**Dependency justification (CLAUDE.md §4.7):** `html2canvas` (`^1.4.1`, MIT license, confirmed via `npm view` before installing) is new. No existing project dependency or browser-native API provides "screenshot the current page as rendered" — `getDisplayMedia()` exists but requires an OS-level screen/tab-picker permission prompt, disproportionately heavy UX for attaching a screenshot to a bug report, and still requires the same canvas-capture work afterward regardless. `html2canvas` is the long-established, actively maintained, zero-runtime-dependency de-facto standard for this exact use case. `npm audit` after installing showed zero new advisories attributable to it (all 16 pre-existing findings — `next`, `xlsx`, `nodemailer`, `postcss`, ESLint tooling, etc. — are unrelated transitive/direct dependencies, confirmed by name, out of scope for this ticket per CLAUDE.md §4.8's separate dependency-update process).

**Design — capture is opt-in and previewed, never automatic, resolving the "do NOT auto-attach Jira data" tension by construction:** a screenshot of a dashboard page necessarily contains Jira-derived content visually, which could read as contradicting the master plan's explicit constraint. The resolution: nothing is captured or attached without the user explicitly clicking "📷 Add a screenshot" inside the already-open feedback modal, and the result is shown as a thumbnail (with a "Remove" option) *before* the form can be submitted — a user-initiated, visually-confirmed capture of what the user is already looking at, categorically different from a silent, automatic data-attachment (which is what the constraint is actually guarding against: leaking structured data into a report without the user's knowledge or consent). `handleCaptureScreenshot()` in `FeedbackButton.tsx` dynamically `import()`s `html2canvas` (code-split — most submissions never trigger it) and captures at `{ scale: 0.6 }`, re-encoding via `canvas.toDataURL('image/jpeg', 0.6)` to keep the payload small; capture failures (e.g. a CSP restriction) show an inline note and never block the rest of the form.

**Storage:** new `Feedback.screenshotData String?` (additive migration `20260802000002_add_feedback_screenshot`) — a plain blob column, deliberately not a new `FeedbackAttachment` table or object-storage service. Mirrors the existing `DashboardSnapshot.metricsJson` precedent (a blob stored directly as a Postgres column, no separate storage layer). A general attachment system would be unwarranted speculative architecture (CLAUDE.md §5.5) for "one optional screenshot per feedback item," which is exactly and only what the tracker line asks for. `POST /api/feedback` enforces a ~2MB hard max length server-side (`MAX_SCREENSHOT_LENGTH`) and rejects — never silently truncates — anything present but malformed (`must start with data:image/`) or oversized.

**Real payload-size risk found and fixed before it shipped, not discovered after:** `GET /api/admin/feedback`'s list endpoint (30 rows/page) previously did a bare `findMany()`; once `screenshotData` existed as a column, that would have shipped up to ~30 × several-hundred-KB base64 blobs in a single admin page-load response. Fixed by switching to an explicit `select` that fetches every existing field plus `screenshotData`, then strips the raw value before the JSON response and replaces it with a cheap computed `hasScreenshot: boolean`. The actual image is fetched lazily, one row at a time, only on admin click, via a new `GET /api/admin/feedback/[id]/screenshot` (same admin-role session check as the existing `[id]/route.ts` PATCH handler, 404 when no screenshot exists). `app/admin/feedback/page.tsx` gained a "📷 View" link (shown only when `hasScreenshot`) that fetches and displays the image in a lightbox rendered via `createPortal(..., document.body)` — avoids invalid `<div>`-inside-`<tbody>` markup that a naive inline render would have produced (a `<tr>` sibling `<div>` is invalid table structure; browsers "foster-parent" such nodes out of the table in unpredictable ways).

**Testing:** extended `src/__tests__/feedbackNotification.test.ts` (the real test file for `POST /api/feedback` despite its name, confirmed via reading it — `TC-FB-01`–`10` already existed) with `TC-FB-11`–`14`: valid screenshot stored on the row; submission without one still succeeds unchanged (regression guard — the new optional field must never become accidentally required); malformed screenshot value rejected 400; oversized screenshot rejected 400. New `src/__tests__/adminFeedback.test.ts` — no positive-path test existed for `GET /api/admin/feedback` before this (only a 403-for-non-admin negative check in `adminNegativeAccess.test.ts`, confirmed via grep): asserts the list response's JSON never contains the string `"base64"` (a direct regression guard against the payload-size risk above, not just a shape check) and does include `hasScreenshot`; the new screenshot sub-route returns the image for an admin, 403 for non-admin (checked before any database query runs), 404 when none exists.

**Verification:** `npx tsc --noEmit` clean (two JSX-fragment closing-tag errors caught and fixed during implementation, before this count — `app/admin/feedback/page.tsx`'s `FeedbackRow` needed a matching `</>` once the lightbox portal was added as a fragment sibling to `<tr>`). `npx eslint` clean, same pre-existing 8-warning baseline — the two new `<img>` tags (capture preview, admin lightbox) are both data-URI renders of locally-produced/stored image data, not optimizable remote assets, so both carry a documented `@next/next/no-img-element` disable comment rather than silently suppressing the rule project-wide. Full Jest suite passing, 129 suites / 1183 tests. `npm run validate` clean. `npx prisma migrate deploy` applied `20260802000002_add_feedback_screenshot` cleanly against the live Postgres (Neon) instance. `npm run build` succeeds; `/api/admin/feedback/[id]/screenshot` present in the route manifest. **Not independently verified, disclosed rather than assumed:** the actual capture/preview/submit flow and the admin lightbox in a live browser — this repo has no jsdom or component-testing infrastructure anywhere (same confirmed gap as every prior P0-B ticket), and canvas rendering specifically isn't meaningfully testable under Node/Jest regardless of that gap. Recommended manual QA before considered fully closed: open the feedback modal on a public page, click "Add a screenshot," confirm a thumbnail preview appears and can be removed/retaken, submit with and without a screenshot, confirm both succeed; in the admin feedback page, confirm the "View" link appears only for submissions with a screenshot and that clicking it loads and displays the image in a lightbox.

---

**FR-417 (P0 — Done, 2026-08-02):** `P0B-05` — the application MUST record versioned product events per the master plan's §4.2 taxonomy, with pseudonymous IDs, schema version, `event_id`, and context fields (§6.2 envelope). Unlike every other P0-B ticket this session, the tracker's "❌ Not started" status was accurate — repo-wide grep confirmed zero prior art (no `anonymous_id`/`pseudonymous`/`session_id`/`event_id`/`schema_version` anywhere in `src`/`app`).

**Scope boundary, held deliberately, not by omission:** the master plan splits this work across three dependent tickets — this one (taxonomy + envelope construction), `P0B-06` (IndexedDB batching queue, flush thresholds, `sendBeacon`), and `P0B-07` (server-side ingestion, validation, dedup). Building all three at once would mean inventing delivery/persistence behavior two tickets ahead of where the tracker places it. Instead, `trackEvent()` (`src/lib/analytics/track.ts`) accepts an injectable transport via `configureAnalyticsTransport()`, defaulting to a `console.debug`-in-development / true-no-op-in-production stub — the SDK is fully real and fully unit-testable today, and P0B-06 replaces the default transport with the real batched one without any call site changing.

**Taxonomy:** `src/lib/analytics/eventTaxonomy.ts` — the master plan's 8 domains (Identity, Upload, Value, Navigation, Feedback, Lifecycle, Quality, Payments) and all 30 named events, verbatim, as a `const` catalog with a derived `AnalyticsEventName` union type and a `Set`-backed `isAnalyticsEventName()` runtime guard (defense-in-depth against a stray string literal reaching `trackEvent()` past the type system).

**Envelope:** `buildEnvelope()` in `track.ts` assembles every §6.2 field — `event_id` (`crypto.randomUUID()`), `schema_version: 1`, `event_name`, `occurred_at` (ISO), `user_id` (nullable, from the existing `currentUser.ts` cache), `anonymous_id`/`session_id` (new — see below), `page` (`window.location.pathname`), optional `section`/`component`, `app_version` (`package.json.version`, same precedent as `app/api/health/route.ts` and `app/api/feedback/route.ts`), `role`, `browser_family`/`browser_major`, `os_family`, `device_category`, optional `result_status`/`duration_ms`, and a generic `properties` bag.

**Consent-gated by design, not bolted on afterward:** `src/lib/analytics/consentGate.ts`'s `getAnalyticsConsent()` checks the analytics-consent toggle `P0B-03` already shipped (`GET/POST /api/consent`) before `trackEvent()`'s transport is ever invoked. The default — unauthenticated, not-yet-decided, or explicitly declined — is always no-op, matching `getConsentStatus()`'s own fail-closed default (CLAUDE.md §38). This is also the direct reason pre-authentication identity events (`signup_started`, pre-session `login_completed`) are **not** wired to any real call site in this ticket: there is no consent mechanism for an anonymous visitor yet — that gap belongs to the master plan's §4.4 GA4/cookie-consent-banner track (`P1-06`, itself already deferred pending exactly this consent record). The taxonomy still defines those event names for P0B-06/07 and future GA4 wiring to consume; they're simply inert until a consent story exists for them. The consent decision is cached client-side (mirroring `currentUser.ts`'s established module-cache pattern, avoiding a network round-trip on every tracked event) and kept in sync at its two natural invalidation points: `PrivacyTab.tsx`'s existing `toggleAnalytics()` now updates the cache immediately on a successful save, and `UserMenu.tsx`'s existing `logout()` clears it alongside its pre-existing `clearCachedUser()` call.

**Pseudonymous/session identity is client-persisted, not server-issued:** `getAnonymousId()` (`localStorage`, long-lived — a pseudonymous identity is meant to survive across browser sessions) and `getSessionId()` (`sessionStorage`, resets per browser session) in the new `src/lib/analytics/clientContext.ts`, both lazily created on first `trackEvent()` call rather than eagerly on page load.

**One real integration point, not a repo-wide instrumentation sweep:** wiring all ~30 taxonomy events into their eventual real call sites (upload flow, dashboard views, navigation, releases) now, while the transport is an inert stub, would touch 10+ files for zero observable benefit and risk overclaiming completeness of a pipeline that doesn't exist yet. Wired exactly `feedback_opened`/`feedback_submitted` into `FeedbackButton.tsx` as the reference integration — proves the full contract end-to-end at the lowest-risk, already-context-rich call site. Broader instrumentation is a named, deliberate deferral: natural work for P0B-06/07 (when a real pipeline exists to feed) or for each feature area as it's next touched, not invented speculatively here.

**No new dependency, matching existing convention rather than CLAUDE.md's general default:** this codebase has no `zod` anywhere (confirmed via repo-wide grep) despite CLAUDE.md's stated preference — `consent.ts` and `feedback/route.ts` both validate with plain `typeof`/membership checks instead. Taxonomy validation here follows suit (a `Set` membership check), and browser/OS/device detection uses no `ua-parser-js` either — it's a small regex extension of `FeedbackButton.tsx`'s pre-existing inline `getBrowserFamily()`, **promoted rather than duplicated** into the new shared `clientContext.ts` (now also returning `browser_major`, `os_family`, `device_category`) — a safe, directly-related dedup while touching this exact area (CLAUDE.md §61), not a scope-creeping refactor. `FeedbackButton.tsx` now imports the shared version instead of its own copy.

**Testing:** new `src/__tests__/analyticsTaxonomy.test.ts` (all 30 event names present across the correct domains, no cross-domain duplicates, `isAnalyticsEventName()` correct for valid/invalid input). New `src/__tests__/analyticsTrack.test.ts` (full envelope shape via an injected test transport — valid UUID `event_id`, ISO `occurred_at`, correct `page`/`role`/`user_id`/browser-OS-device fields; no transport call when consent isn't granted or the session is unauthenticated; unknown event names dropped without invoking the transport; `anonymous_id`/`session_id` stable across repeated calls within a session; `event_id` unique per call). New `src/__tests__/analyticsConsentGate.test.ts` (no fetch when no session is cached; fetches `/api/consent` exactly once and caches the result; a fetch failure resolves to `false` rather than rejecting; `setAnalyticsConsentCache`/`clearAnalyticsConsentCache` behave correctly). 17 new test cases across the three files. No component test for `FeedbackButton.tsx`'s new `trackEvent()` calls or the `PrivacyTab.tsx`/`UserMenu.tsx` cache wiring — same disclosed, pre-existing no-jsdom-infrastructure gap as every prior P0-B ticket.

**Verification:** `npx tsc --noEmit` clean. `npx eslint` clean, same pre-existing 8-warning baseline, no new warnings (no `no-console` rule is configured in this repo — confirmed via `.eslintrc.json` and existing precedent in `src/lib/logger.ts` — so the SDK's dev-only `console.debug`/`console.warn` calls needed no suppression comment). Full Jest suite passing, 132 suites / 1200 tests. `npm run validate` clean. `npm run build` succeeds. No Prisma migration in this ticket — zero server-side persistence, by design (decision 1 above). **Not independently verified, disclosed rather than assumed:** the live-browser behavior of the `console.debug` output and the Settings → Privacy toggle immediately starting/stopping it — same disclosed no-jsdom-infrastructure gap as every prior P0-B ticket. Recommended manual QA before considered fully closed: with `npm run dev` running and signed in, open the feedback modal and confirm a well-formed `feedback_opened`/`feedback_submitted` envelope logs to the console only when analytics consent is granted in Settings → Privacy; toggle it off and confirm the console output stops immediately; confirm no output at all while signed out.

---

**FR-418 (P0 — Done, 2026-08-03):** `P0B-06` — queued analytics events MUST be delivered in 10-second batches (or at a 20–50 event threshold, on connectivity return, on page-hidden, or on a critical error) via a durable IndexedDB queue, per master plan §4.7, with delivery via `sendBeacon` for exit payloads and `fetch`/`keepalive` otherwise, deleting only server-acknowledged events. Confirmed a genuinely fresh subsystem — repo-wide grep found zero `indexedDB`/`sendBeacon`/`visibilitychange` usage anywhere before this ticket, unlike almost every other P0-B ticket this session.

**Module split (CLAUDE.md §5.1):** `src/lib/analytics/eventQueue.ts` is a pure IndexedDB CRUD adapter (`enqueueEvent`/`getQueuedEvents`/`deleteEvents`/`countQueuedEvents`/`pruneExpiredEvents`/`enforceMaxQueueSize`) with no network or timer code — independently unit-testable against a real (simulated) IndexedDB. `src/lib/analytics/eventFlush.ts` owns triggers, delivery, and retry, importing the queue rather than touching storage directly. `trackEvent()`'s default transport (P0B-05's inert `console.debug` stub) is swapped for "enqueue to IndexedDB" via the existing `configureAnalyticsTransport()` extension point — the exact swap point P0B-05 was designed to leave open. Wired once from a new `src/components/ui/AnalyticsQueueInit.tsx`, deliberately a new sibling component rather than extending the existing `GlobalErrorHandler.tsx` — keeps each global-init component single-purpose (CLAUDE.md §5.1) instead of growing an existing, differently-named one into a second unrelated responsibility. Mounted in `app/layout.tsx` alongside it.

**Scope tension with `P0B-07`, resolved with the user before implementation, not assumed:** the master plan splits client queueing (`P0B-06`) from server ingestion — "validation, rate limits, accepted/rejected event ID response, deduplication and storage" (`P0B-07`) — as separate, dependent tickets. But this ticket's own core requirement, "delete only server-acknowledged events," is unverifiable without a real endpoint to acknowledge against. Asked the user directly rather than assuming either a scope-creeping full endpoint or an untestable mock-only queue; decided to build a minimal, real `POST /api/events` in this ticket implementing the master plan's exact `{accepted, rejected}` response contract — rate-limited via the same `LoginAttempt`-table sliding-window pattern the pre-existing `POST /api/events/error` already uses (own key prefix `ev:`, a higher 120-per-15-min ceiling reflecting that one flush can carry up to 50 events and the endpoint is polled every 10s while active), with per-event validation (`event_id` present, `schema_version` a number, `event_name` a member of `isAnalyticsEventName()` from `eventTaxonomy.ts`, safely reusable server-side since it's pure TS with no browser API) — but persisting nothing: no new Prisma model, no migration. A code comment in the route states plainly that `P0B-07` adds real validation depth, deduplication, and durable storage behind this same contract; the route's own header comment also warns not to assume events reaching it are stored anywhere.

**Also confirmed via research, not touched:** `src/lib/errorReporter.ts`/`app/api/events/error/route.ts` is a separate, already-fully-built, single-event (not batched) pipeline tagged `P0B-08` (structured error monitoring) — unrelated to this ticket's general product-event queue. Its rate-limiting helper shape is reused directly (own copy in the new route, not a shared extraction — two call sites don't yet meet CLAUDE.md §5.4's Rule of Three for a shared abstraction).

**Flush triggers** (`eventFlush.ts`'s `initAnalyticsQueue()`): a 10-second `setInterval`; immediately when the queue reaches a 30-event threshold (within the master plan's 20–50 range) right after an enqueue; the browser's `online` event; `visibilitychange` when `document.hidden` becomes true; and immediately on enqueueing any quality-domain event (`client_error`/`api_error`) — the master plan's "critical error" trigger, interpreted as "don't make a real-time error signal wait up to 10 seconds."

**A real correctness point the master plan's own two rules don't reconcile — found and resolved during design, not left implicit:** `navigator.sendBeacon()` has no response callback, so the page-hidden/exit flush path cannot know what the server accepted. Deleting on send anyway would silently violate "delete only server-acknowledged events" for exactly the moment (a closing tab) recovery matters least. Resolution: the beacon path is genuinely fire-and-forget and never deletes; those events remain queued and are safely re-flushed — and only then deleted, on a real acknowledgement — by the next `fetch`-based flush (the next page load's initial flush, or the 10s interval if the tab doesn't actually close). All non-beacon triggers use `fetch(..., { keepalive: true })` and delete strictly based on the parsed `{accepted, rejected}` response.

**Retry/backoff redesigned mid-implementation after a real bug surfaced while writing tests, not as originally planned:** the first implementation scheduled retries via a self-scheduling `setTimeout` with exponential backoff. Writing `analyticsEventFlush.test.ts` surfaced that this is a genuine timer outliving a test's synchronous assertions — it fired mid-suite into a later, unrelated test with whatever `fetch` mock was then installed, corrupting results and hanging the run. `jest.useFakeTimers()` was tried as a fix and made things worse: `fake-indexeddb` (the new devDependency added for this ticket) turns out to depend on real timers internally to simulate IndexedDB's async request/transaction model, so faking timers broke every IndexedDB operation, timing out every test in the file. Root-caused rather than worked around: redesigned backoff as a passive gate — a `nextAttemptAllowedAt` timestamp checked at the top of `flushQueue()` (`Date.now() < nextAttemptAllowedAt` → skip), doubling on failure (base 5s, cap 5min, ±10% jitter) and reset on success — no `setTimeout` in the retry path at all. This is simpler than the original design, not just test-friendlier: the existing periodic/online/threshold/critical triggers already re-attempt regularly, making a second, independent self-scheduling mechanism redundant. `rejected` event_ids are deleted alongside `accepted` ones (a `rejected` event, e.g. `invalid_schema`, will never succeed on retry — keeping it queued would only accumulate dead weight). `MAX_QUEUE_SIZE = 500` (oldest dropped first), `MAX_EVENT_AGE_MS = 24h` (pruned each flush attempt), and a 50-event per-batch cap (client-enforced in `getQueuedEvents(50)`, and re-enforced server-side in `POST /api/events` as defense in depth against a client-side cap bypass) bound queue growth — analytics is best-effort, never critical data.

**Dependency justification (CLAUDE.md §4.7):** `fake-indexeddb` (`^6.2.5`, Apache-2.0, confirmed via `npm view` before installing) is new, **devDependency only** — zero production/bundle impact. No existing project dependency or Node built-in provides IndexedDB, and this ticket's central deliverable (queue persistence, delete-on-ack, expiry/size eviction) would otherwise ship with zero automated coverage — a materially bigger gap than the peripheral UI-wiring gaps disclosed in prior tickets, since IndexedDB correctness is the actual thing this ticket is named after. `npm audit` after installing showed the same 16 pre-existing findings as before, none attributable to `fake-indexeddb` by name (confirmed via `npm audit | grep fake-indexeddb`, no matches). No new production dependency for IndexedDB access itself — a single object store with put/getAll/delete/count is small enough that raw `indexedDB` in a ~90-line adapter didn't warrant `idb` or a similar wrapper, matching P0B-05's "no new dependency where reasonably avoidable" precedent.

**Testing:** new `src/__tests__/analyticsEventQueue.test.ts` (8 cases, real `fake-indexeddb`-backed IndexedDB — enqueue/get round-trip, delete-by-id precision, count accuracy, expiry pruning by cutoff, oldest-first max-size eviction, no-op edge cases). New `src/__tests__/analyticsEventFlush.test.ts` (6 cases, `jest.resetModules()` + dynamic imports per test so module-level retry/backoff state and the simulated database are both fresh each test — successful flush deletes accepted+rejected while keeping unacknowledged events queued; network failure and non-2xx responses both leave events queued without throwing; the beacon path calls `sendBeacon` and deletes nothing; an empty queue never calls `fetch`; a 55-event queue sends at most 50 per flush). New `src/__tests__/eventsApi.test.ts` (7 cases — well-formed batch fully accepted; an unknown `event_name` is rejected while valid siblings in the same batch are still accepted; missing `event_id`/`schema_version` rejected with specific reasons; non-array body 400; a 60-event batch truncated to the server-side 50 cap; rate limiting trips at the configured threshold before any processing). 21 new cases across the three files. No test for `AnalyticsQueueInit.tsx`'s mount wiring or real browser `visibilitychange`/`online` event firing — same disclosed, pre-existing no-jsdom-infrastructure gap as every prior P0-B ticket.

**Verification:** `npx tsc --noEmit` clean (two spread-argument type errors on jest mock signatures caught and fixed during implementation, before this count). `npx eslint` clean, same pre-existing 8-warning baseline, no new warnings. Full Jest suite passing, 135 suites / 1221 tests. `npm run validate` clean. `npm run build` succeeds; `/api/events` present in the route manifest alongside the pre-existing `/api/events/error`. No Prisma migration in this ticket — zero server-side persistence, by design (the scope-tension resolution above). **Not independently verified, disclosed rather than assumed:** live-browser behavior of the queue (IndexedDB accumulation in DevTools' Application panel, actual flush timing, real offline/online transitions, a real beacon firing on tab close) — same disclosed no-jsdom-infrastructure gap as every prior P0-B ticket, and browser-specific APIs (`sendBeacon`, `visibilitychange`, real `indexedDB`) specifically aren't meaningfully exercisable under Node/Jest regardless of that gap. Recommended manual QA before considered fully closed: with `npm run dev` running and signed in with analytics consent granted, open DevTools → Application → IndexedDB and confirm `dc-analytics-queue` accumulates events while navigating, drains within ~10s or at the 30-event threshold, and that `POST /api/events` in the Network tab returns `{accepted, rejected}`; toggle DevTools' offline mode and confirm events queue without errors and flush automatically once back online; confirm a beacon fires on tab close/hide.
