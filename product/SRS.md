...
**Orphaned subsystem — flagged, not deleted:** the entire prior Role-Based Coaching generator/orchestrator/confidence/trend/evidence-link subsystem (7 generators, orchestrator, `ceremonyAdvice.service.ts`, `coachingConfidence.service.ts`, `coachingTrend.service.ts`, `coachingEvidenceLink.ts`, `coachingBadge.ts`, `adminSignals.service.ts`, `app/api/coaching/admin-signals/route.ts`, `src/types/roleBasedCoaching.ts`) now has zero callers from any page or component — confirmed by repo-wide grep excluding each file's own tests. This is real, tested, still-plausibly-reusable domain logic, not accidental dead code from a rename; deleting ~15 files and their test suites was judged a larger and more unrelated action than the redesign the user asked for, so it was left in place and tracked as `ORPHAN-03` in `TODO-List.md` Section 18f for an explicit future decision, per the same CLAUDE.md §5 "no unowned code left undecided" principle already applied to `ORPHAN-01`/`ORPHAN-02`. `CoachingInsightCard.tsx`/`CoachingOtherRoles.tsx` — pure presentational components with no domain logic and, after this change, zero remaining callers — were deleted outright rather than flagged, consistent with how `CoachingCategoryTabs.tsx` was handled in Addendum AC's sibling change hours earlier.
