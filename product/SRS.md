...
- **Dashboard routed pages** — dashboard refactored into 10 independent Next.js App Router pages under `app/dashboard/[section]/`: priority-attention (incl. Smart Actions), key-metrics, data-quality, delivery-composition, trends (sprint + quarter, toggle), ownership (ownership & capacity), labels, epic-readiness, flow-health, coaching. `kanban-health`, `visual-analytics`, and `delivery-controls` were removed 2026-07-11 in a nav consolidation (100% duplicate content elsewhere); a same-day follow-up pass then merged `actions` into `priority-attention` and merged `sprint-status`+`quarter-statistics` into `trends`. All five merged-away routes redirect to their replacement.
