...
**Verification:** 1 new test (`register.test.ts` — self-registration creates the user with `dataStorageMode: "local"` and `role: "user"`); full suite 854/92 passing, no regressions. `npx tsc --noEmit` clean throughout. The new tab UI itself (Settings shell, tab switching, role-gating) has no automated test coverage — this repo has no component-testing infrastructure (no jsdom/RTL wired into `jest.config.js`, consistent with the same limitation noted in Addendum N) — recommended manual check before shipping: log in as each role, confirm the Settings side menu renders correctly and only the intended tabs appear.
