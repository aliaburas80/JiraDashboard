...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { q: "Why don't I see all 7 coaching categories?", a: 'Coaching content is grouped into 7 categories, but most roles only see the one that matches them: Scrum Master, Product Owner, and C-level each see exactly one, shown directly with no extra navigation. The Manager role sees three related categories (Engineering Manager, Delivery Manager, Team Lead), and Admins see all 7 — for those roles, the rest of your categories collapse under a "View other roles" section below your main card instead of cluttering the page.' },
      ...
      { q: 'Which category is shown by default, and what’s in "View other roles"?', a: 'When you have more than one visible category, they’re automatically sorted so the most urgent one comes first and is shown by default — that’s the card you see immediately. Expanding "View other roles" lists the rest with a mood icon and one-line summary each; selecting one swaps it into the main card above.' },
      { q: 'What does the small up/down badge next to the mood label mean?', a: 'It shows whether that category has improved, worsened, or stayed the same compared to your most recently saved snapshot (Snapshots page). It only appears once you have saved at least two snapshots — there is nothing to compare against before that.' },
      { q: 'Can I click on the evidence numbers?', a: 'Some evidence chips (the small boxes showing a number and label) are clickable and take you straight to the dashboard page where that number comes from — for example, an Average Cycle Time chip links to Flow Health. Chips without an arrow icon are not linked, but you can still hover them for the full detail.' },
    ],
  },
  {
