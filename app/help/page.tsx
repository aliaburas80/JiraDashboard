...
const SECTIONS: Section[] = [
  {
    ...
    items: [
      ...
      { q: 'Step 3 — Read the dashboard top to bottom', a: 'The dashboard opens on Priority Attention (default), which also includes the Smart Actions recommendations. Use the left sidebar to navigate between 9 pages — Overview group (Priority Attention, Key Metrics, Data Quality — which also includes the Delivery Composition breakdown), Delivery group (Trends), and Deep Dive group (Ownership & Capacity, Labels & Types, Epic Readiness, Flow Health Table, Coaching Insights). The 52px topbar shows the product health pill and a New Upload button.' },
  ...
  {
    id: 'controls', icon: 'settings', title: 'Flow Efficiency & Story Points',
    items: [
      { q: 'Flow Efficiency', a: 'Flow Efficiency = (active work time / total elapsed time) × 100. A score above 40% is generally considered healthy for software teams. Anything below 25% indicates items spend most of their life waiting — in queues, in review, or blocked — rather than being actively worked on. This lives on the dashboard\'s Key Metrics page.' },
      ...
      { q: 'Critical signals', a: 'Critical signals are binary indicators — Blocked items present (yes/no), Overdue items present (yes/no), No-sprint orphans present (yes/no), and No-assignee items present (yes/no) — surfaced on the dashboard\'s Priority Attention page. Any active signal is highlighted in red and links directly to the matching filtered view in the flow table.' },
    ],
  },
  {
  ...
  {
    id: 'coaching', icon: 'lightbulb', title: 'Team Role View',
    items: [
      { q: 'What is the Team Role View?', a: 'The Team Role View page (/dashboard/coaching) is a simple, light grid with one column each for Scrum Master, Product Owner, and Manager. Each column shows only what that role needs to watch: its rules to monitor, its current actions, and its key measures — no tabs, no charts, no clicking to see another role, since all three are shown at once.' },
      { q: 'What is a "rule to monitor," and what do the status pills mean?', a: 'Each role has a short list of process rules (e.g. "Blocked work must be reviewed daily"). Every rule shows a status pill — Critical, At risk, Review, or Healthy — reflecting whether that rule currently has something to flag in your uploaded data. A couple of rules (retrospective action ownership, priority-change visibility) always show Healthy, since the app doesn\'t yet track that specific information — see "Are all the numbers real?" below.' },
      { q: 'What are "Current actions"?', a: 'A short, curated list of the next concrete steps for that role — e.g. reviewing overdue delivery dates, or escalating unresolved dependencies. Where a real count is available (like the number of critical epics), the action text uses it directly rather than a fixed example number.' },
      { q: 'Are all the numbers real?', a: 'Most of them are computed directly from your uploaded Jira data — blocked items, overdue items, orphan items, critical epics, delivery health score, and more. A few (carry-over rate, sprint goal coverage, forecast variance) are simple derived percentages of existing numbers, not new calculations. Exactly one metric, "Retro actions completed" under Scrum Master, has no real data behind it yet (retrospective action ownership isn\'t tracked anywhere in the app) and shows a clearly-labeled placeholder value until that tracking exists.' },
      { q: 'What do the specific threshold numbers mean?', a: 'Three rules compare a computed value against a fixed threshold, stated directly in the rule\'s own description on the page: Scrum Master\'s "Carry-over must remain below the agreed threshold" flags At risk above 20% of committed sprint items carried over — a placeholder default, not yet a team-configurable setting; Manager\'s "Capacity overload must be corrected" flags Risk when one team member carries more than 35% load share, but only once the team has more than 2 members (the same rule the coaching insight logic uses elsewhere in the app); Manager\'s "Delivery forecasts must be credible" flags Critical below 60% overall delivery confidence. The Glossary\'s "Analytics Scores & UI Terms" section documents the same three figures for reference.' },
      { q: 'Why only 3 roles — what about C-level or Admin?', a: 'This page intentionally covers the three roles with the most day-to-day, rule-driven delivery decisions. It isn\'t personalized to your own account role — everyone who opens the page sees the same three columns.' },
    ],
  },
  {
    ...
    items: [
      {
        ...
        a: (
          <ol className={styles.stepList}>
            <li>In Jira, navigate to your project and open the <strong className={styles.strongLight}>Backlog</strong> view (not the Board view).</li>
            <li>Click the three-dot menu (<strong className={styles.strongLight}>…</strong>) in the top-right corner of the backlog.</li>
            <li>Choose <strong className={styles.strongLight}>Export</strong> → <strong className={styles.strongLight}>Export Excel XML</strong> (for .xls) or <strong className={styles.strongLight}>Export to CSV (all fields)</strong>.</li>
            <li>If prompted to select columns, ensure the recommended columns below are included.</li>
            <li>Save the file to your computer. The file will be named something like <em className={styles.emphasisMuted}>Jira.csv</em> or <em className={styles.emphasisMuted}>Jira.xls</em>.</li>
            ...
            <table className={styles.docTable}>
              <thead>
                <tr className={styles.docTableHeadRow}>
                  <th className={styles.docTableHeadCell}>Canonical field</th>
                  <th className={styles.docTableHeadCell}>Accepted aliases</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={canonical} className={styles.docTableRow}>
                    <td className={styles.docTableKeyCell}>{canonical}</td>
                    <td className={styles.docTableCell}>{aliases}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        a: (
          <div className={styles.tableWrap}>
            <table className={styles.docTable}>
              <thead>
                <tr className={styles.docTableHeadRow}>
                  <th className={styles.docTableHeadCell}>Method</th>
                  <th className={styles.docTableHeadCell}>Route</th>
                  <th className={styles.docTableHeadCell}>Description</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={route} className={styles.docTableRow}>
                    <td className={styles.docTableCellTight}>
                      <span className={styles.methodBadge} data-method={method}>{method}</span>
                    </td>
                    <td className={styles.docTableKeyCell}>{route}</td>
                    <td className={styles.docTableDescCell}>{desc}</td>
                  </tr>
                ))}
              </tbody>
            </table>
    ...
    items: [
      {
        ...
        a: (
          <div className={styles.stackSm}>
            <p className={styles.mutedPara}>Every page shows a <strong className={styles.strongLight}>data source badge</strong> in the top navigation bar indicating where the current data came from:</p>
            <ul className={styles.plainList}>
              <li className={styles.mutedListItem}><span className={styles.providerLabel} data-provider="s3"><SvgIcon name="cloud" size={12} /> S3</span> — loaded or cached from Amazon S3</li>
              <li className={styles.mutedListItem}><span className={styles.providerLabel} data-provider="azure"><SvgIcon name="cloud" size={12} /> Azure</span> — loaded or cached from Azure Blob Storage</li>
              <li className={styles.mutedListItem}><span className={styles.providerLabel} data-provider="gcp"><SvgIcon name="globe" size={12} /> GCP</span> — loaded or cached from Google Cloud Storage</li>
              <li className={styles.mutedListItem}><span className={styles.providerLabel}><SvgIcon name="save" size={12} /> Local cache</span> — cloud provider is set but data served from local cache (no re-fetch needed)</li>
              <li className={styles.mutedListItem}><span className={styles.providerLabel} data-provider="jira"><SvgIcon name="upload" size={12} /> Jira upload</span> — data came from a fresh Jira CSV upload in this browser session</li>
              <li className={styles.mutedListItem}><span className={styles.providerLabel} data-provider="fallback"><SvgIcon name="warning" size={12} /> localStorage fallback</span> — bucket/server metrics were unavailable; data came from this browser&apos;s saved fallback copy</li>
            </ul>
            <p className={clsx(styles.mutedPara, styles['mutedPara--spaced'])}>When data is actively loading from the cloud, a blue <strong className={styles.strongLight}>loading banner</strong> appears at the top of the page: <em>&ldquo;Loading data from Amazon S3…&rdquo;</em></p>
          </div>
        ),
      },
  ...
  },
  {
    ...
    items: [
      { q: 'What is the Cloud Storage feature?', a: 'The Cloud Storage tab in /admin/settings allows admins to configure a cloud provider (AWS S3, Azure Blob, or Google Cloud Storage) to receive automatic backup files. Backups include the SQLite database, config files, import logs, and each workspace\'s own dashboard metrics file so the dashboard can load from the bucket-backed server copy on the next session. This is a separate, admin-only, whole-app backup destination — not the same thing as an individual user\'s "Data Storage Mode" (see below), which controls where that one user\'s own Jira uploads go.' },
  ...
  {
    id: 'data-storage-mode', icon: 'shield', title: 'Data Storage Mode (App Storage vs. This Device Only)',
    items: [
      { q: 'What is Data Storage Mode?', a: 'Go to /profile (Settings) → Storage to choose where your uploaded Jira data and computed dashboard metrics live: "App storage" — stored in Delivery Clarity\'s own server database, not a third-party cloud service — or "This device only" — your file is parsed and your metrics are calculated entirely inside your own browser and are never uploaded to the server at all. New self-registered accounts start on "This device only"; accounts an admin creates for their team start on "App storage" so the team has shared visibility. Either way, you can switch anytime.' },
      ...
      { q: 'Are health thresholds and orphan-issue rules the same in "This device only" mode?', a: 'Local-mode calculations use Delivery Clarity\'s built-in default thresholds and orphan-issue rules, not any custom values an admin has configured for your organization, since those live on the server. App storage mode always reflects the current admin-configured values.' },
      { q: 'Can I still see my past local uploads?', a: 'Yes — /profile (Settings) → Storage\'s Import History list shows your local uploads too (stored in your browser, capped at your 20 most recent). Use "Delete all history" there, or "Clear Local Data" on the upload page, to remove them.' },
      { q: 'Why did my "App storage" dashboard suddenly show as empty on 2026-07-05?', a: 'A one-time fix: each App-storage user\'s dashboard now loads from a file scoped to their own workspace instead of one file shared across every App-storage user. The old shared file is still on the server but is no longer read by anyone, since automatically guessing which user it belonged to could have shown you someone else\'s data. Simply upload your Jira export (or sync Jira) again to repopulate your own dashboard — nothing else about your account changed.' },
      { q: 'Can I use my own cloud bucket instead of App storage?', a: 'Yes — while in "App storage" mode, /profile (Settings) → Storage has an optional "Your own cloud bucket" section where you can connect your own Amazon S3, Azure Blob Storage, or Google Cloud Storage bucket. Your credentials are encrypted and used only for your own uploads — never shared with any other user, and never routed through Delivery Clarity\'s own bucket.' },
      { q: 'Why are my uploads blocked after I saved my own bucket?', a: 'A saved bucket has to pass "Test connection" before it\'s actually used — this is deliberate: rather than silently falling back to App storage (or guessing), uploads are blocked with a clear message until your bucket is verified. Go to Settings → Storage and click "Test connection," or click "Remove" to go back to App storage immediately.' },
      { q: 'What happens to data I already uploaded if I connect my own bucket later?', a: 'Nothing moves automatically — connecting your own bucket only affects your next upload once it\'s verified. Existing App-storage data stays in App storage.' },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      { q: 'What is the About (Landing) page?', a: 'The /landing page is an in-app product showcase. It has a full-screen animated hero with quick links to the upload page and dashboard, a diagram of how a Jira export becomes delivery decisions, key product stats, a "How it works" section, all 12 major features as clickable cards, a business-value section, an illustrative dashboard preview, and a branded CTA footer with an option to try the sample dataset. A left-side progress rail (desktop only) lets you jump between sections. Access it via Reference → About in the nav, or click "See everything it does →" on the upload page.' },
    ...
    items: [
      {
        ...
        a: (
          <div className={styles.stackXs}>
            <p className={styles.sectionLabel}>Main workbook — triggered from the Export button or Overview page:</p>
            <div className={styles.tightStack}>
              {[
                ...
                <div key={sheet} className={styles.sheetRow}>
                  <span className={clsx(styles.sheetName, styles['sheetName--wide'])}>{sheet}</span>
                  <span className={styles.sheetDesc}>{desc}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      },
      {
        ...
        a: (
          <div className={styles.stackXs}>
            <p className={styles.sectionLabel}>Explorer workbook — triggered from the Export button on /explore after a graph is loaded:</p>
            <div className={styles.tightStack}>
              {[
                ...
                <div key={sheet} className={styles.sheetRow}>
                  <span className={clsx(styles.sheetName, styles['sheetName--narrow'])}>{sheet}</span>
                  <span className={styles.sheetDesc}>{desc}</span>
                </div>
              ))}
            </div>
  ...
  },
  {
    ...
    items: [
      ...
      { q: 'I signed up myself — do I need to verify my email?', a: "You'll receive a verification email with a link (it goes to /verify-email) — clicking it confirms your address. You can sign in and use your account before verifying, but Delivery Clarity shows a reminder banner and an \"!\" badge on your avatar until you do, so it's worth doing early to keep your account secure. The link works from any browser or device, not just the one you registered on." },
      { q: 'I never received my verification email — what do I do?', a: 'Check your spam folder first. If it is missing, sign in and use the "Resend verification email" button in the reminder banner shown at the top of the app. The verification link expires 24 hours after it is sent.' },
...

  {
    ...
    items: [
      { q: 'What does the Retrospective page do?', a: 'The Retro page (/retro) is a sprint retrospective tool. You can fill out a retrospective directly in the app, download an Excel template to fill offline, or upload a completed file for automated analysis. Both the in-app form and uploaded files use the same insights engine.' },
      ...
      { q: 'What insights does the app generate?', a: 'For each sprint, the app detects common themes (process, communication, requirements, QA/release, dependency, technical, planning) from your "What Did Not Go Well" entries and blockers — never from positive feedback, so praise is never mistaken for a problem. It also flags action items missing an owner or due date, flags duplicate action items, and generates next-sprint suggestions citing the specific blocker count or theme that triggered them — never generic advice.' },
      { q: 'Does the app suggest actual stories or tasks for next sprint, not just advice?', a: 'Yes. The "Suggested Stories & Tasks for Next Sprint" section on each insight panel shows concrete, copy-pasteable items: a blocker becomes a task to resolve it; a blocker that keeps recurring becomes a spike to investigate the root cause instead of trying the same fix again; your most common concern this sprint becomes a story, citing the exact comment that triggered it; a missed sprint goal becomes a spike to investigate why. Click "Copy" on any item to paste it into your own backlog.' },
      { q: 'Can I create these suggested items directly in Jira?', a: 'Not yet. Each suggestion has a disabled "Create in Jira (coming soon)" button next to Copy — this app currently has read-only Jira access, so direct ticket creation is a planned future capability (tracked as FUT-JIRA-02), not something it does today.' },
      { q: 'How do I download the retro template?', a: 'Click "Download .xlsx →" on the Download Template card on the Retro landing page. The file Retrospective_Template.xlsx downloads with a "Retrospective" sheet (4 example rows) and an "Instructions" sheet. Prefer CSV? Click the smaller "or download as .csv instead" link underneath.' },
      { q: 'How do I upload a completed retrospective file?', a: 'Click "Upload →" on the Retro landing page, then choose a .csv, .xlsx, .xls, .md, or .txt file (up to 5 MB). The app parses it into one retrospective per sprint and shows the same insights as the in-app form for every sprint found, plus any import warnings.' },
      { q: 'What happens if my upload has multiple sprints?', a: 'If the same blocker text appears in more than one sprint in your file, it is flagged as a "repeated" blocker on every sprint where it occurs — useful for spotting a standing issue that keeps coming up across retros instead of three unrelated one-off problems.' },
      { q: 'Is my uploaded retrospective saved anywhere?', a: 'No. Uploaded files are parsed in memory to build the preview you see and are not persisted. Re-uploading the same file produces the same preview again from scratch.' },
    ],
  },
  {
...
function AccordionItem({ item, isOpen, onToggle, query }: {
  ...
  return (
    <div className={styles.accordionItemWrap} data-open={isOpen}>
      <button
      ...
      {isOpen && (
        <div className={styles.accordionBody}>
          {typeof item.a === 'string' ? (
            ...
            <div className={styles.accordionAnswerNode}>{item.a}</div>
          )}
        </div>
      )}
    </div>
...
function SectionCard({ section, expandedItem, onToggleItem, forceOpen = false, query }: {
  ...
  return (
    <div className={styles.sectionCard} data-open={isOpen}>
      <button
        className={styles.sectionCardButton}
        onClick={() => setOpen(v => !v)}
        aria-expanded={isOpen}
      >
        <SvgIcon name={section.icon} size={18} className={styles.sectionCardIcon} />
        ...
        <span className={styles.sectionCardCaret} aria-hidden>▾</span>
      </button>
      {isOpen && (
    ...
    <AppShell showNav>
      <div className={styles.page}>
        ...
        <div className={styles.hero}>
          <span className={clsx('chip c-acc', styles.heroChip)}>HELP &amp; DOCS</span>
        ...
        {/* Search */}
        <div id="tour-section-help-search" className={styles.searchWrap}>
          ...
          <div className={styles.heroCardGrid}>
            {HERO_CARDS.map(card => {
              ...
              return (
                ...
                >
                  <SvgIcon name={card.icon} size={22} className={styles.heroCardIcon} />
                  <div className={styles.heroCardTitle}>{card.title}</div>
                  <div className={styles.heroCardDesc}>{card.desc}</div>
                  <div className={styles.heroCardPills}>
                    {pills.map(t => (
                      <span
                        key={t}
                        className={clsx('chip c-nt', styles.heroCardPill)}
                      >{t}</span>
                    ))}
                  </div>
                </button>
        ...
        {/* Group Filter Pills */}
        {!isSearching && (
          <div id="tour-section-help-pills" className={styles.groupPills}>
            {[{ id: 'all', label: 'All topics', icon: 'book' }, ...SECTION_GROUPS].map(g => {
              ...
              return (
                <button
                  ...
                  className={clsx(isActive ? 'chip c-acc' : 'chip c-nt', styles.groupPill)}
                  data-active={isActive}
                >
        ...
        {visibleSections.length === 0 ? (
          <div className={styles.emptyState}>
            <SvgIcon name="search" size={40} className={styles.emptyStateIcon} />
            <div className={styles.emptyStateTitle}>No results for &ldquo;{search}&rdquo;</div>
            <div className={styles.emptyStateBody}>Try a different keyword or clear the search box.</div>
          </div>
        ) : (
