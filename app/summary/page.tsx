...
import { PageLoading } from '@/components/dashboard/DashboardPageShell';
    ...
    <div className="px-7 pt-6 pb-12">
      ...
      <section className="mb-7" aria-labelledby="kpi-heading">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <KpiCard label="Completion"   value={`${metrics.completionRate ?? 0}%`}           detail={`${metrics.doneIssues ?? 0} of ${metrics.totalIssues ?? 0} issues`}          accent="#16a34a" />
          <KpiCard label="Health Alerts" value={riskItems}                                   detail={`${flow.critical ?? 0} critical · ${flow.warning ?? 0} warning`}              accent="#dc2626" />
          <KpiCard label="Active Work"   value={metrics.activeIssues ?? 0}                   detail="In Progress, Code Review, QA, UAT"                                            accent="#d97706" />
          <KpiCard label="Lead Time"     value={`${flow.averageLeadTimeDays ?? 0}d`}         detail={`${flow.leadTimeSampleSize ?? 0} completed items`}                            accent="#2563eb" />
          <KpiCard label="Cycle Time"    value={`${flow.averageCycleTimeDays ?? 0}d`}        detail={`${flow.cycleTimeSampleSize ?? 0} items with start dates`}                    accent="#0f766e" />
          {(sp.totalStoryPoints ?? 0) > 0 ? (
            <KpiCard label="Story Points" value={`${sp.pointCompletionRate ?? 0}%`}          detail={`${sp.completedStoryPoints ?? 0} / ${sp.totalStoryPoints} pts`}              accent="#7c3aed" />
          ) : (
            <KpiCard label="Story Points" value="—"                                          detail="No story points in this export"                                               accent="#94a3b8" />
          )}
        </div>
      </section>
            ...
            {topBlockers.length > 0 && (
              <div className="flex flex-col gap-1.5 bg-red-50 border border-red-200 rounded-xl p-4 shadow-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-red-800">Blocker{topBlockers.length !== 1 ? 's' : ''}</span>
                </div>
            ...
            {orphanCount > 0 && (
              <div className="flex flex-col gap-1.5 bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-slate-700">Orphan{orphanCount !== 1 ? 's' : ''}</span>
                </div>
