...
// Colors match the Mediterranean palette (THEME-05) — previously a bespoke
// dark background with the old orange brand accent.
...
// Brand palette (THEME-05) — teal weighted double because it is the primary
// accent (Mediterranean Intelligence), matching AnimatedDataBackground's palette.
const PALETTE = [
  [8, 127, 140],   // teal (accent)
  [8, 127, 140],   // teal (extra weight)
  [44, 123, 229],  // blue (acc2)
  [139, 92, 246],  // purple (decorative variety, kept from original)
  [32, 48, 56],    // dark text-tone, for depth against the light background
] as const;
...
function spawnDrop(w: number, h: number, scatter: boolean): Drop {
  return {
    ...
    // Softer range than the old dark-bg tuning — full 0.18-0.70 alpha would
    // read too heavy against a light background.
...
export function LegalRainBackground({ className }: Props) {
  ...
  useEffect(() => {
    ...
    function drawFrame() {
      ...
      // Light radial gradient (THEME-05) — lighter at centre to draw the eye inward
      ...
      ctx!.fillStyle = 'rgba(32,48,56,0.04)';
      const step = 32;
      ...
      vig.addColorStop(0,    'rgba(32,48,56,0)');
      vig.addColorStop(0.6,  'rgba(32,48,56,0)');
      vig.addColorStop(1,    'rgba(32,48,56,0.10)');
      ctx!.fillStyle = vig;
      ctx!.fillRect(0, 0, w, h);
