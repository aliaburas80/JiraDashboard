...
export default function BackupRestoreSettings({ files }: Props) {
  ...
  function showMsg(m: string) { setMsg(m); setTimeout(() => setMsg(''), 5000); }

  async function handleBackup() {
    setCreating(true); setMsg('');
    try {
      const res  = await fetch('/api/admin/backup');
      if (!res.ok) { showMsg('Backup failed.'); return; }
    ...
    } catch { showMsg('Backup failed — check console.'); }
    finally { setCreating(false); }
  }

  async function handleRestore(file: File) {
    setRestoring(true); setMsg(''); setRestoreResult(null);
    try {
      ...
      if (data.ok) showMsg(`✓ Restore complete. ${data.restored.length} file(s) restored.`);
      else showMsg(`Restore completed with errors. Check details below.`);
    } catch { showMsg('Restore failed — invalid backup file.'); }
          ...
          <div className="mt-4 space-y-2">
            {restoreResult.restored.length > 0 && (
              <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3">
                <p className="text-xs font-black text-green-800 mb-1">✓ Restored</p>
            ...
            {restoreResult.errors.length > 0 && (
              <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3">
                <p className="text-xs font-black text-red-800 mb-1">✗ Errors</p>
