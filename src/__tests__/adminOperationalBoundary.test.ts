...
describe('EP-024 separate Admin operational boundary', () => {
  ...
  test('user-app Admin cutover preserves the selected Admin operation and rejects same-origin misconfiguration', () => {
    ...
    expect(proxy).toContain('base.origin === req.nextUrl.origin');
    expect(proxy).toContain("unavailable.searchParams.set('adminUnavailable', '1')");
